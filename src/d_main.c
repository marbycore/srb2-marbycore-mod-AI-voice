...
static void D_Display(void) {
  ...
  if (wipe && wipetypepre != INT16_MAX) {
    ...
    if (rendermode != render_none) {
      ...
      if ((wipegamestate == (gamestate_t)FORCEWIPE ||
           (wipegamestate != (gamestate_t)FORCEWIPEOFF &&
            !(gamestate == GS_LEVEL ||
              (gamestate == GS_TITLESCREEN &&
               titlemapinaction)))) // fades to black on its own timing, always
        ...
        F_WipeEndScreen();
        F_RunWipe(wipetypepre,
                  gamestate != GS_TIMEATTACK && gamestate != GS_TITLESCREEN);
      }
  ...
  if (wipegamestate == GS_INTRO && gamestate == GS_TITLESCREEN)
    ...
    if (gamestate == GS_LEVEL ||
        (gamestate == GS_TITLESCREEN && titlemapinaction && curbghide &&
         (!hidetitlemap))) {
      ...
      if (!automapactive && !dedicated && cv_renderview.value) {
        R_ApplyLevelInterpolators(R_UsingFrameInterpolation() ? rendertimefrac
                                                              : FRACUNIT);
      ...
      if (lastdraw) {
        if (rendermode == render_soft) {
          VID_BlitLinearScreen(screens[0], screens[1], vid.width * vid.bpp,
                               vid.height, vid.width * vid.bpp, vid.rowbytes);
  ...
  if (!wipe) {
    if (cv_netstat.value) {
      ...
      V_DrawRightAlignedString(BASEVIDWIDTH, BASEVIDHEIGHT - 40, V_YELLOWMAP,
                               s);
      snprintf(s, sizeof s - 1, "send %d b/s", sendbps);
      V_DrawRightAlignedString(BASEVIDWIDTH, BASEVIDHEIGHT - 30, V_YELLOWMAP,
                               s);
      snprintf(s, sizeof s - 1, "GameMiss %.2f%%", gamelostpercent);
      V_DrawRightAlignedString(BASEVIDWIDTH, BASEVIDHEIGHT - 20, V_YELLOWMAP,
                               s);
      snprintf(s, sizeof s - 1, "SysMiss %.2f%%", lostpercent);
      V_DrawRightAlignedString(BASEVIDWIDTH, BASEVIDHEIGHT - 10, V_YELLOWMAP,
                               s);
    }
...
#ifdef __EMSCRIPTEN__
  EM_ASM(
      try { StartedMainLoopCallback(); } catch(err) {
...
static void D_RunFrame(void) {
  ...
  if (!D_LockFrame) {
...

    {
      ...
      double budget =
          round((1.0 / R_GetFramerateCap()) * I_GetPrecisePrecision());
    ...
    if (realtics > 0 || singletics) {
      ...
      if (lastdraw || singletics || gametic > rendergametic) {
        ...
        doDisplay = true;
                 ...
                 entertic) // in case the server hang or netsplit
      {
        // Lagless camera! Yay!
        if (gamestate == GS_LEVEL && netgame) {
          ...
          for (tic_t chasecamtics = 0; chasecamtics < realtics;
               chasecamtics++) {
            if (splitscreen && camera2.chase)
              P_MoveChaseCamera(&players[secondarydisplayplayer], &camera2,
                                false);
            if (camera.chase)
    ...
    if (interp) {
      ...
      ps_interp_frac.value.p =
          (precise_t)((FIXED_TO_FLOAT(g_time.timefrac)) * 1000.0f);
    ...
    if (!singletics) {
      ...
      const boolean vsync_with_match_refresh =
          cv_vidwait.value && cv_fpscap.value == 0;

      if (elapsed > 0 && (INT64)capbudget > elapsed &&
          !vsync_with_match_refresh) {
        I_SleepDuration(capbudget - (finishprecise - enterprecise));
      }
    }
    // Capture the time once more to get the real delta time.
    finishprecise = I_GetPreciseTime();
    deltasecs = (double)((INT64)(finishprecise - enterprecise)) /
...
void D_StartTitle(void) {
  ...
  players[consoleplayer].availabilities = players[1].availabilities =
      R_GetSkinAvailabilities(); // players[1] is supposed to be for 2p
  ...
  // empty maptol so mario/etc sounds don't play in sound test when they
  // shouldn't
  maptol = 0;
  ...
  if (tutorialmode && tutorialgcs) {
    G_CopyControls(gamecontrol, gamecontroldefault[gcs_custom],
                   gcl_tutorial_full,
                   num_gcl_tutorial_full); // using gcs_custom as temp storage
    ...
    M_StartMessage("Do you want to \x82save the recommended \x82movement "
                   "controls?\x80\n\nPress 'Y' or 'Enter' to confirm\nPress "
                   "'N' or any key to keep \nyour current controls",
...
#define REALLOC_FILE_LIST                                                      \
  if (list->files == NULL) {                                                   \
    ...
    index = list->numfiles;                                                    \
    list->files =                                                              \
        realloc(list->files, sizeof(list->files) * ((++list->numfiles) + 1));  \
    if (list->files == NULL)                                                   \
...
///\brief Checks if a netgame URL is being handled, and changes working
/// directory to the EXE's if so.
///       Done because browsers (at least, Firefox on Windows) launch the game
///       from the browser's directory, which causes problems.
static void ChangeDirForUrlHandler(void) {
  // URL handlers are opened by web browsers (at least Firefox) from the
  // browser's working directory, not the game's stored directory, so chdir to
  // that directory unless overridden.
  if (M_GetUrlProtocolArg() != NULL && !M_CheckParm("-nochdir")) {
    size_t i;

    CONS_Printf("%s connect links load game files from the SRB2 application's "
                "stored directory. Switching to ",
                SERVER_URL_PROTOCOL);
...
static void IdentifyVersion(void) {
  ...
  snprintf(configfile, sizeof configfile, "%s" PATHSEP CONFIGFILENAME,
           srb2waddir);
  ...
  // if you change the ordering of this or add/remove a file, be sure to update
  // the md5 checking in D_SRB2Main
...
static void D_ConvertVersionNumbers(void) {
...
static void LaunchTelemetryDashboard(void) {
  char cwd[1024];
  const char *exes[] = {"TelemetryDashboard.exe", "TelemetryDashboardV2.exe"};
  const char *paths[] = {".", "..", "../..", "../../..", "../../../.."};
  int i, j;
  boolean success = false;

  if (_getcwd(cwd, sizeof(cwd)) != NULL)
    CONS_Printf("Current working directory: %s\n", cwd);

  CONS_Printf("Attempting to launch Telemetry Dashboard...\n");

  for (j = 0; j < 5; j++) // Loop through paths
  {
    for (i = 0; i < 2; i++) // Loop through exe names
    {
      char fullPath[MAX_PATH];
      snprintf(fullPath, sizeof(fullPath), "%s\\%s", paths[j], exes[i]);
      ...
      if (GetFileAttributesA(fullPath) != INVALID_FILE_ATTRIBUTES) {
        ...
        sei.lpDirectory =
            NULL; // Use current directory or maybe the dir of the exe?
        // Better to set directory to where the dashboard is to avoid issues
        sei.nShow = SW_SHOW;
        sei.hInstApp = NULL;

        if (ShellExecuteExA(&sei)) {
          CONS_Printf("Successfully launched %s from %s\n", exes[i], paths[j]);
          success = true;
  ...
  if (!success) {
    CONS_Printf("Could not find or launch TelemetryDashboard.exe in any "
                "expected location.\n");
  }
}
#endif
...
void D_SRB2Main(void) {
  ...
  strcpy(liveeventbackup,
         "live" SAVEGAMENAME ".bkp"); // intentionally not ending with .ssg

  {
    ...
    !defined(__CYGWIN__)
      ...
      snprintf(downloaddir, sizeof downloaddir, "%s" PATHSEP "DOWNLOAD",
               srb2home);
      if (dedicated)
        snprintf(configfile, sizeof configfile, "%s" PATHSEP "d" CONFIGFILENAME,
                 srb2home);
      else
        snprintf(configfile, sizeof configfile, "%s" PATHSEP CONFIGFILENAME,
                 srb2home);
      ...
      snprintf(luafiledir, sizeof luafiledir, "%s" PATHSEP "luafiles",
               srb2home);
#else  // DEFAULTDIR
  ...
  // Do this up here so that WADs loaded through the command line can use
  // ExecCfg
  COM_Init();
  ...
  // adapt tables to SRB2's needs, including extra slots for dehacked file
  // support
  P_PatchInfoTables();
...
#endif
  // don't check music.pk3 because people like to modify it, and it doesn't
  // matter if they do
  // ...except it does if they slip maps in there, and that's what
  // W_VerifyNMUSlumps is for.
  ...
  D_RegisterClientCommands(); // be sure that this is called before
                              // D_CheckNetGame
  ...
  // to avoid setting off the corrupted gamedata code in G_LoadGameData if a SOC
  // with custom gamedata is added
  ...
  CONS_Printf("AI_Init(): Init AI system.\n");
  AI_Init();
  COM_BufInsertText("addfile ai_logic.lua\n");
  ...
  if (M_CheckParm("-noaudio")) // combines -nosound and -nomusic
  {
    ...
    if (M_CheckParm("-nomusic")) // combines -nomidimusic and -nodigmusic
    {
      ...
      if (M_CheckParm("-nomidimusic"))
        midi_disabled =
            true; // WARNING: DOS version initmusic in I_StartupSound
  ...
  if (M_CheckParm("-room")) {
    if (!M_IsNextParm())
      I_Error("usage: -room <room_id>\nCheck the Master Server's webpage for "
              "room ID numbers.\n");
  ...
  if (dedicated)
    ...
    COM_ImmedExecute(
        va("exec \"%s" PATHSEP "autoexec.cfg\" -noerror\n", srb2home));
  ...
  if (autostart || netgame) {
    ...
    // Do this here so if you run SRB2 with eg +timelimit 5, the time limit
    // counts as having been modified for the first game.
    M_PushSpecialParameters(); // push all "+" parameter at the command buffer

    COM_BufExecute(); // ensure the command buffer gets executed before the map
                      // starts (+skin)
    ...
    if (M_CheckParm("-gametype") && M_IsNextParm()) {
      ...
      {
        j = atoi(sgametype); // assume they gave us a gametype number, which is
                             // okay too
...

  {
    ...
    CV_SetValue(&cv_ai_controlled, 1);
    CV_SetValue(&cv_pauseifunfocused, 0);
  }
...
const char *D_Home(void) {
  ...
  if (!userhome && usehome) // Alam: Still not?
  {
    ...
    if (testhome != NULL &&
        (FIL_FileOK(va("%s" PATHSEP "%s" PATHSEP CONFIGFILENAME, testhome,
                       DEFAULTDIR)))) {
      userhome = testhome;
    }
  }
...
boolean D_IsPathAllowed(const char *path) {
  char *paths[] = {srb2home, srb2path, cv_addons_folder.zstring};
