...
// deh_tables.c now has lists for the more named enums! PLEASE keep them up to
// date! For great modding!!
...
// Hey, moron! If you change this table, don't forget about sprnames in info.c
// and the sprite lights in hw_light.c!
typedef enum sprite {
...
typedef enum playersprite {
  ...
  SPR2_XTRA, // stuff that isn't in-map - "would this ever need an md2 or
             // variable length animation?"

  SPR2_FIRSTFREESLOT,
  SPR2_LASTFREESLOT =
      1024, // Do not make higher than SPR2F_MASK (currently 0x3FF) plus one
  NUMPLAYERSPRITES
} playersprite_t;

enum { XTRA_LIFEPIC, XTRA_CHARSEL, XTRA_CONTINUE, XTRA_ENDING };
...
typedef enum state {
  ...
  S_CYBRAKDEMON_FINISH_ATTACK1, // If just attacked, remove MF2_FRET w/out going
                                // back to spawnstate
  S_CYBRAKDEMON_FINISH_ATTACK2, // Force a delay between attacks so you don't
                                // get bombarded with them back-to-back
  S_CYBRAKDEMON_PAIN1,
  S_CYBRAKDEMON_PAIN2,
  S_CYBRAKDEMON_PAIN3,
  S_CYBRAKDEMON_DIE1,
...
typedef struct {
  spritenum_t sprite;
  UINT32 frame; // we use the upper 16 bits for translucency and other shade
                // effects
  INT32 tics;
  actionf_p1 action;
  INT32 var1;
