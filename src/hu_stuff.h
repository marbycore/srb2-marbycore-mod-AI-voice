...
#define CHAT_BUFSIZE                                                           \
  64 // that's enough messages, right? We'll delete the older ones when that
     // gets out of hand.
#ifdef NETSPLITSCREEN
#define OLDCHAT (cv_consolechat.value == 1 || dedicated || vid.width < 640)
#else
#define OLDCHAT                                                                \
  (cv_consolechat.value == 1 || dedicated || vid.width < 640 || splitscreen)
#endif
#define CHAT_MUTE                                                              \
  ((cv_mute.value || players[consoleplayer].muted) &&                          \
   !(server ||                                                                 \
     IsPlayerAdmin(                                                            \
         consoleplayer))) // this still allows to open the chat but not to type.
                          // That's used for scrolling and whatnot.
#define OLD_MUTE                                                               \
  (OLDCHAT && (cv_mute.value || players[consoleplayer].muted) &&               \
   !(server || IsPlayerAdmin(consoleplayer))) // this is used to prevent oldchat
                                              // from opening when muted.
...
void HU_drawPing(INT32 x, INT32 y, UINT32 ping, boolean notext,
                 INT32 flags); // Lat': Ping drawer for scoreboard.
