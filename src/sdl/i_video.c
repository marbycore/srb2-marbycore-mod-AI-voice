...
#include "SDL_image.h"
    ...
    (!defined(__APPLE__) &&                                                    \
     defined(UNIXCOMMON)) // Windows & Mac don't need this, as SDL will do it
                          // for us.
...
consvar_t cv_vidwait =
    CVAR_INIT("vid_wait", "On", CV_SAVE | CV_CALL, CV_OnOff, VidWaitChanged);
static consvar_t cv_stretch =
    CVAR_INIT("stretch", "Off", CV_SAVE | CV_NOSHOWHELP, CV_OnOff, NULL);
static consvar_t cv_alwaysgrabmouse =
    CVAR_INIT("alwaysgrabmouse", "Off", CV_SAVE, CV_OnOff, NULL);
...
#define USE_FULLSCREEN                                                         \
  (disable_fullscreen || !allow_fullscreen) ? 0 : cv_fullscreen.value
static SDL_bool disable_mouse = SDL_FALSE;
#define USE_MOUSEINPUT (!disable_mouse && cv_usemouse.value && havefocus)
#define MOUSE_MENU                                                             \
  false //(!disable_mouse && cv_usemouse.value && menuactive && !USE_FULLSCREEN)
...
static void SDLSetMode(INT32 width, INT32 height, SDL_bool fullscreen,
  ...
  if (window) {
    ...
    } else // windowed mode
    {
      ...
      if (reposition) {
        ...
        // Force maximize after initial positioning/sizing
  ...
  if (rendermode == render_soft) {
    ...
    texture = SDL_CreateTexture(renderer, sw_texture_format,
    ...
    SDL_PixelFormatEnumToMasks(sw_texture_format, &bpp, &rmask, &gmask, &bmask,
                               &amask);
    vidSurface =
        SDL_CreateRGBSurface(0, width, height, bpp, rmask, gmask, bmask, amask);
  }
}
...
#ifdef HWRENDER
  else if (rendermode == render_opengl && sdlglcontext != NULL &&
           SDL_GL_GetCurrentContext() == sdlglcontext) {
...
static void SDLdoGrabMouse(void) {
  ...
  if (SDL_SetRelativeMouseMode(SDL_TRUE) ==
      0) // already warps mouse if successful
    wrapmouseok =
...
// SDL2 doesn't have SDL_GetVideoSurface or a lot of the SDL_Surface flags that
// SDL 1.2 had
static void SurfaceInfo(const SDL_Surface *infoSurface,
                        const char *SurfaceText) {
  INT32 vfBPP;
...
static void VID_Command_Mode_f(void) {
  ...
  if (COM_Argc() != 2) {
    CONS_Printf(
        M_GetText(
            "vid_mode <modenum> : set video mode, current video mode %i\n"),
        vid.modenum);
    return;
  }
...
static inline void SDLJoyRemap(event_t *event) { (void)event; }
...
static INT32 SDLJoyAxis(const Sint16 axis, evtype_t which) {
  ...
  if (which == ev_joystick) {
    if (Joystick.bGamepadStyle) {
      ...
      raxis = JoyInfo.scale != 1 ? ((raxis / JoyInfo.scale) * JoyInfo.scale)
                                 : raxis;
    ...
    if (Joystick2.bGamepadStyle) {
      ...
      raxis = JoyInfo2.scale != 1 ? ((raxis / JoyInfo2.scale) * JoyInfo2.scale)
                                  : raxis;
...
static void Impl_HandleWindowEvent(SDL_WindowEvent evt) {
  ...
  if (mousefocus && kbfocus) {
    ...
    if (!cv_playmusicifunfocused.value && !cv_ai_controlled.value)
      S_PauseAudio();
    if (!cv_playsoundsifunfocused.value && !cv_ai_controlled.value)
      S_StopSounds();
...
static void Impl_HandleMouseButtonEvent(SDL_MouseButtonEvent evt, Uint32 type) {
  ...
  // this apparently makes a mouse button down event but not a mouse button up
  // event, resulting in whatever key was pressed down getting "stuck" if we
  // don't ignore it.
...
static void Impl_HandleJoystickButtonEvent(SDL_JoyButtonEvent evt,
                                           Uint32 type) {
  event_t event;
  SDL_JoystickID joyid[2];
...
void I_GetEvent(void) {
  ...
  while (SDL_PollEvent(&evt)) {
    ...
    case SDL_JOYDEVICEADDED: {
      ...
      // Because SDL's device index is unstable, we're going to cheat here a
      // bit: For the first joystick setting that is NOT active:
      // 1. Set cv_usejoystickX.value to the new device index (this does not
      // change what is written to config.cfg)
      // 2. Set OTHERS' cv_usejoystickX.value to THEIR new device index, because
      // it likely changed
      //    * If device doesn't exist, switch cv_usejoystick back to default
      //    value (.string)
      //      * BUT: If that default index is being occupied, use ANOTHER
      //      cv_usejoystick's default value!
          ...
          JoyInfo2.dev != newjoy) // don't override a currently active device
      {
        ...
        if (JoyInfo2.dev)
          ...
          cv_usejoystick2.value = 0;
                 ...
                 (!JoyInfo2.dev || !SDL_JoystickGetAttached(JoyInfo2.dev)) &&
                 JoyInfo.dev !=
                     newjoy) // don't override a currently active device
      {
      ...
      if (!strcmp(cv_usejoystick.string, "0") || !cv_usejoystick.value)
        cv_usejoystick.value = 0;
      else if (atoi(cv_usejoystick.string) <=
                   I_NumJoys() // don't mess if we intentionally set higher than
                               // NumJoys
               &&
      ...
      if (!strcmp(cv_usejoystick2.string, "0") || !cv_usejoystick2.value)
        cv_usejoystick2.value = 0;
      else if (atoi(cv_usejoystick2.string) <=
                   I_NumJoys() // don't mess if we intentionally set higher than
                               // NumJoys
               &&
      ...
      // we need to do this in case CV_SetValue did nothing because the string
      // was already same. if the device is already active, this should do
      // nothing, effectively.
      I_InitJoystick();
      I_InitJoystick2();
    ...
    case SDL_JOYDEVICEREMOVED:
      ...
      // * If device doesn't exist, switch cv_usejoystick back to default value
      // (.string)
      //   * BUT: If that default index is being occupied, use ANOTHER
      //   cv_usejoystick's default value!
      if (JoyInfo.dev)
        cv_usejoystick.value = JoyInfo.oldjoy =
            I_GetJoystickDeviceIndex(JoyInfo.dev) + 1;
      else if (atoi(cv_usejoystick.string) != JoyInfo2.oldjoy)
        cv_usejoystick.value = atoi(cv_usejoystick.string);
      else if (atoi(cv_usejoystick2.string) != JoyInfo2.oldjoy)
        cv_usejoystick.value = atoi(cv_usejoystick2.string);
      ...
      if (JoyInfo2.dev)
        cv_usejoystick2.value = JoyInfo2.oldjoy =
            I_GetJoystickDeviceIndex(JoyInfo2.dev) + 1;
      else if (atoi(cv_usejoystick2.string) != JoyInfo.oldjoy)
        cv_usejoystick2.value = atoi(cv_usejoystick2.string);
      else if (atoi(cv_usejoystick.string) != JoyInfo.oldjoy)
        cv_usejoystick2.value = atoi(cv_usejoystick.string);
...
static inline boolean I_SkipFrame(void) {
  ...
  case GS_LEVEL:
    if (!paused)
      return false;
    /* FALLTHRU */
  // case GS_TIMEATTACK: -- sorry optimisation but now we have a cool level
  // platter and that being laggardly looks terrible
...
void I_FinishUpdate(void) {
  ...
  if (rendermode == render_soft && screens[0]) {
    ...
    if (bufSurface) {
      ...
      // Fury -- there's no way around UpdateTexture, the GL backend uses it
      // anyway
      SDL_LockSurface(vidSurface);
      SDL_UpdateTexture(texture, &src_rect, vidSurface->pixels,
                        vidSurface->pitch);
    ...
    // Final postprocess step of palette rendering, after everything else has
    // been drawn.
...
void I_ReadScreen(UINT8 *scr) {
  if (rendermode != render_soft)
    ...
    VID_BlitLinearScreen(screens[0], scr, vid.width * vid.bpp, vid.height,
...
void I_SetPalette(RGBA_t *palette) {
  ...
  // if (vidSurface) SDL_SetPaletteColors(vidSurface->format->palette,
  // localPalette, 0, 256);
  //  Fury -- SDL2 vidSurface is a 32-bit surface buffer copied to the texture.
  //  It's not palletized, like bufSurface.
  if (bufSurface)
...
const char *VID_GetModeName(INT32 modeNum) {
  ...
  sprintf(&vidModeName[modeNum][0], "%dx%d", windowedModes[modeNum][0],
...
void VID_PrepareModeList(void) {
  // Under SDL2, we just use the windowed modes list, and scale in windowed
  // fullscreen.
...
#ifdef HWRENDER
  if (vid.glstate ==
      VID_GL_LIBRARY_ERROR) // Well, it didn't work the first time anyway.
  {
...
boolean VID_CheckRenderer(void) {
  ...
  SDLSetMode(vid.width, vid.height, USE_FULLSCREEN,
             (setmodeneeded ? SDL_TRUE : SDL_FALSE));
...
static SDL_bool Impl_CreateWindow(SDL_bool fullscreen) {
  ...
  if (rendermode == render_none) // dedicated
    return SDL_TRUE; // Monster Iestyn -- not sure if it really matters what we
                     // return here tbh
  ...
  // Create a window
  window = SDL_CreateWindow("SRB2 " VERSIONSTRING, SDL_WINDOWPOS_CENTERED,
                            SDL_WINDOWPOS_CENTERED, realwidth, realheight,
...
static void Impl_VideoSetupSDLBuffer(void) {
  ...
  // Set up the SDL palletized buffer (copied to vidbuffer before being rendered
  // to texture)
  if (vid.bpp == 1) {
    bufSurface = SDL_CreateRGBSurfaceFrom(
        screens[0], vid.width, vid.height, 8, (int)vid.rowbytes, 0x00000000,
        0x00000000, 0x00000000, 0x00000000); // 256 mode
  } else if (vid.bpp == 2) // Fury -- don't think this is used at all anymore
  {
    bufSurface = SDL_CreateRGBSurfaceFrom(
        screens[0], vid.width, vid.height, 15, (int)vid.rowbytes, 0x00007C00,
        0x000003E0, 0x0000001F, 0x00000000); // 555 mode
  }
  if (bufSurface) {
    SDL_SetPaletteColors(bufSurface->format->palette, localPalette, 0, 256);
  } else {
...
#endif
  {
    ...
    if (vd &&
        (strncasecmp(vd, "gcvideo", 8) == 0 ||
         strncasecmp(vd, "fbcon", 6) == 0 || strncasecmp(vd, "wii", 4) == 0 ||
...
void VID_StartupOpenGL(void) {
  ...
  if (!glstartup) {
    ...
    vid.glstate = HWD.pfnInit()
                      ? VID_GL_LIBRARY_LOADED
                      : VID_GL_LIBRARY_ERROR; // let load the OpenGL library
...
void I_GetCursorPosition(INT32 *x, INT32 *y) { SDL_GetMouseState(x, y); }
...
void EMSCRIPTEN_KEEPALIVE unlock_mouse(void) { SDLforceUngrabMouse(); }

void EMSCRIPTEN_KEEPALIVE lock_mouse(void) { SDLdoGrabMouse(); }
#endif
