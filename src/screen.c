...
// ------------------
viddef_t vid;
INT32 setmodeneeded; // video mode change needed if > 0 (the mode number to set
                     // + 1)
...
consvar_t cv_scr_width =
    CVAR_INIT("scr_width", "1280", CV_SAVE, CV_Unsigned, NULL);
consvar_t cv_scr_height =
    CVAR_INIT("scr_height", "800", CV_SAVE, CV_Unsigned, NULL);
consvar_t cv_scr_width_w =
    CVAR_INIT("scr_width_w", "640", CV_SAVE, CV_Unsigned, NULL);
consvar_t cv_scr_height_w =
    CVAR_INIT("scr_height_w", "400", CV_SAVE, CV_Unsigned, NULL);
consvar_t cv_scr_depth =
    CVAR_INIT("scr_depth", "16 bits", CV_SAVE, scr_depth_cons_t, NULL);

CV_PossibleValue_t cv_renderer_t[] = {{1, "Software"},
#ifdef HWRENDER
                                      {2, "OpenGL"},
#endif
                                      {0, NULL}};

consvar_t cv_renderer = CVAR_INIT("renderer", "Software", CV_SAVE | CV_CALL,
                                  cv_renderer_t, SCR_ChangeRenderer);

static void SCR_ChangeFullscreen(void);

consvar_t cv_fullscreen = CVAR_INIT("fullscreen", "No", CV_SAVE | CV_CALL,
                                    CV_YesNo, SCR_ChangeFullscreen);
...
void SCR_SetDrawFuncs(void) {
  ...
  if (vid.bpp == 1) // Always run in 8bpp.
  {
    ...
    spanfuncs[SPANDRAWFUNC_TILTEDTRANSSPRITE] =
        R_DrawTiltedTranslucentFloorSprite_8;
    ...
    spanfuncs[SPANDRAWFUNC_TILTEDTRANSSOLID] =
        R_DrawTiltedTransSolidColorSpan_8;
    spanfuncs[SPANDRAWFUNC_WATERSOLID] = R_DrawWaterSolidColorSpan_8;
    spanfuncs[SPANDRAWFUNC_TILTEDWATERSOLID] =
        R_DrawTiltedWaterSolidColorSpan_8;
    ...
    spanfuncs_npo2[SPANDRAWFUNC_TILTEDTRANS] =
        R_DrawTiltedTranslucentSpan_NPO2_8;
    spanfuncs_npo2[SPANDRAWFUNC_SPLAT] = R_DrawSplat_NPO2_8;
    spanfuncs_npo2[SPANDRAWFUNC_TRANSSPLAT] = R_DrawTranslucentSplat_NPO2_8;
    spanfuncs_npo2[SPANDRAWFUNC_TILTEDSPLAT] = R_DrawTiltedSplat_NPO2_8;
    spanfuncs_npo2[SPANDRAWFUNC_TILTEDTRANSSPLAT] =
        R_DrawTiltedTranslucentSplat_NPO2_8;
    spanfuncs_npo2[SPANDRAWFUNC_SPRITE] = R_DrawFloorSprite_NPO2_8;
    spanfuncs_npo2[SPANDRAWFUNC_TRANSSPRITE] =
        R_DrawTranslucentFloorSprite_NPO2_8;
    spanfuncs_npo2[SPANDRAWFUNC_TILTEDSPRITE] = R_DrawTiltedFloorSprite_NPO2_8;
    spanfuncs_npo2[SPANDRAWFUNC_TILTEDTRANSSPRITE] =
        R_DrawTiltedTranslucentFloorSprite_NPO2_8;
...
void SCR_Recalc(void) {
  ...
  // toggle off (then back on) the automap because some screensize-dependent
  // values will be calculated next time the automap is activated.
  if (automapactive) {
    am_recalc = true;
    AM_Start();
  }
...
void SCR_CheckDefaultMode(void) {
  ...
  if (scr_forcex && scr_forcey) {
    ...
    if (cv_fullscreen.value)
      setmodeneeded =
          VID_GetModeForSize(cv_scr_width.value, cv_scr_height.value) +
          1; // see note above
    else
      setmodeneeded =
          VID_GetModeForSize(cv_scr_width_w.value, cv_scr_height_w.value) +
          1; // see note above

    if (setmodeneeded <= 0)
      CONS_Alert(CONS_WARNING,
                 "Invalid resolution given, defaulting to base resolution\n");
  }
...
void SCR_SetDefaultMode(void) {
  ...
  CV_SetValue(cv_fullscreen.value ? &cv_scr_height : &cv_scr_height_w,
              vid.height);
}
...
void SCR_ChangeFullscreen(void) {
  ...
  if (graphics_started) {
    ...
    if (cv_fullscreen.value)
      ...
      setmodeneeded =
          VID_GetModeForSize(cv_scr_width_w.value, cv_scr_height_w.value) + 1;
...
void SCR_ChangeRenderer(void) {
...
#ifdef HWRENDER
  // Check if OpenGL loaded successfully (or wasn't disabled) before switching
  // to it.
  ...
  if (rendermode == render_opengl &&
      (vid.glstate ==
       VID_GL_LIBRARY_LOADED)) // Clear these out before switching to software
...
void SCR_DisplayTicRate(void) {
  ...
  if (cap > 0) {
    ...
    else if (fps <= cap * 0.90)
      ticcntcolor = V_YELLOWMAP;
    else
  ...
  if (cv_ticrate.value == 2) // compact counter
  {
    V_DrawRightAlignedString(
        vid.width, h, ticcntcolor | V_NOSCALESTART | V_USERHUDTRANS,
        va("%04.2f", averageFPS));  // use averageFPS directly
  } else if (cv_ticrate.value == 1) // full counter
  {
    ...
    V_DrawString(vid.width - ((7 * 8 * vid.dup) +
                              V_StringWidth("FPS: ", V_NOSCALESTART)),
                 h, V_YELLOWMAP | V_NOSCALESTART | V_USERHUDTRANS, "FPS:");
...
void SCR_DisplayLocalPing(void) {
  ...
  if (cv_showping.value == 1 ||
      (cv_showping.value == 2 && servermaxping &&
...
void SCR_ClosedCaptions(void) {
    ...
    if (promptactive)
      ...
      basey -= 8;
             ...
             ((cv_powerupdisplay.value == 2) // "Always"
              || (cv_powerupdisplay.value == 1 &&
                  !camera.chase))) // "First-person only"
      basey -= 16;
  }
  ...
  for (i = 0; i < NUMCAPTIONS; i++) {
    ...
    if (music && !gamestopped && (closedcaptions[i].t < flashingtics) &&
        (closedcaptions[i].t & 1))
      continue;
    ...
    if (closedcaptions[i].b) {
      ...
      if (closedcaptions[i]
              .b) // If the caption hasn't reached its final destination...
      {
        y -= closedcaptions[i].b * 4 * FRACUNIT; // ...move it per tic...
        y += (rendertimefrac % FRACUNIT) * 4; // ...and interpolate it per frame
        // We have to modulo it by FRACUNIT, so that it won't be a tic ahead
        // with interpolation disabled Unlike everything else, captions are
        // (intentionally) interpolated from T to T+1 instead of T-1 to T
      }
    }
    ...
    V_DrawRightAlignedStringAtFixed(
        (BASEVIDWIDTH - 20) * FRACUNIT, y, flags,
        va("%c [%s]", dot,
           (closedcaptions[i].s->caption[0] ? closedcaptions[i].s->caption
                                            : closedcaptions[i].s->name)));
  }
}
...
#define PRIMEV2 17 // I can't believe it! I'm on TV!
    ...
    str = va("%i:%02i:%02i.%02i", G_TicsToHours(marathontime),
             G_TicsToMinutes(marathontime, false),
             G_TicsToSeconds(marathontime), G_TicsToCentiseconds(marathontime));
    oldentertics = entertic;
  }
  V_DrawFill((antisplice[0] / PRIMEV1) - 1, BASEVIDHEIGHT - 8, 1, 8,
             V_SNAPTOBOTTOM | V_SNAPTOLEFT);
  V_DrawFill((antisplice[0] / PRIMEV1), BASEVIDHEIGHT - 8, 1, 8,
             V_SNAPTOBOTTOM | V_SNAPTOLEFT | 31);
  V_DrawFill(BASEVIDWIDTH - ((antisplice[1] / PRIMEV1) - 1), BASEVIDHEIGHT - 8,
             1, 8, V_SNAPTOBOTTOM | V_SNAPTORIGHT);
  V_DrawFill(BASEVIDWIDTH - ((antisplice[1] / PRIMEV1)), BASEVIDHEIGHT - 8, 1,
             8, V_SNAPTOBOTTOM | V_SNAPTORIGHT | 31);
#undef PRIMEV1
#undef PRIMEV2
