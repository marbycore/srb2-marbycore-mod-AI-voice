...
static CV_PossibleValue_t ticrate_cons_t[] = {
    {0, "No"}, {1, "Full"}, {2, "Compact"}, {0, NULL}};
consvar_t cv_ticrate =
    CVAR_INIT("showfps", "No", CV_SAVE, ticrate_cons_t, NULL);

static void CV_palette_OnChange(void);

static CV_PossibleValue_t gamma_cons_t[] = {
    {-15, "MIN"}, {5, "MAX"}, {0, NULL}};
consvar_t cv_globalgamma = CVAR_INIT("gamma", "0", CV_SAVE | CV_CALL,
                                     gamma_cons_t, CV_palette_OnChange);

static CV_PossibleValue_t saturation_cons_t[] = {
    {0, "MIN"}, {10, "MAX"}, {0, NULL}};
consvar_t cv_globalsaturation =
    CVAR_INIT("saturation", "10", CV_SAVE | CV_CALL, saturation_cons_t,
              CV_palette_OnChange);

#define huecoloursteps 4

static CV_PossibleValue_t hue_cons_t[] = {
    {0, "MIN"}, {(huecoloursteps * 6) - 1, "MAX"}, {0, NULL}};
consvar_t cv_rhue =
    CVAR_INIT("rhue", "0", CV_SAVE | CV_CALL, hue_cons_t, CV_palette_OnChange);
consvar_t cv_yhue =
    CVAR_INIT("yhue", "4", CV_SAVE | CV_CALL, hue_cons_t, CV_palette_OnChange);
consvar_t cv_ghue =
    CVAR_INIT("ghue", "8", CV_SAVE | CV_CALL, hue_cons_t, CV_palette_OnChange);
consvar_t cv_chue =
    CVAR_INIT("chue", "12", CV_SAVE | CV_CALL, hue_cons_t, CV_palette_OnChange);
consvar_t cv_bhue =
    CVAR_INIT("bhue", "16", CV_SAVE | CV_CALL, hue_cons_t, CV_palette_OnChange);
consvar_t cv_mhue =
    CVAR_INIT("mhue", "20", CV_SAVE | CV_CALL, hue_cons_t, CV_palette_OnChange);

consvar_t cv_rgamma = CVAR_INIT("rgamma", "0", CV_SAVE | CV_CALL, gamma_cons_t,
                                CV_palette_OnChange);
consvar_t cv_ygamma = CVAR_INIT("ygamma", "0", CV_SAVE | CV_CALL, gamma_cons_t,
                                CV_palette_OnChange);
consvar_t cv_ggamma = CVAR_INIT("ggamma", "0", CV_SAVE | CV_CALL, gamma_cons_t,
                                CV_palette_OnChange);
consvar_t cv_cgamma = CVAR_INIT("cgamma", "0", CV_SAVE | CV_CALL, gamma_cons_t,
                                CV_palette_OnChange);
consvar_t cv_bgamma = CVAR_INIT("bgamma", "0", CV_SAVE | CV_CALL, gamma_cons_t,
                                CV_palette_OnChange);
consvar_t cv_mgamma = CVAR_INIT("mgamma", "0", CV_SAVE | CV_CALL, gamma_cons_t,
                                CV_palette_OnChange);

consvar_t cv_rsaturation = CVAR_INIT("rsaturation", "10", CV_SAVE | CV_CALL,
                                     saturation_cons_t, CV_palette_OnChange);
consvar_t cv_ysaturation = CVAR_INIT("ysaturation", "10", CV_SAVE | CV_CALL,
                                     saturation_cons_t, CV_palette_OnChange);
consvar_t cv_gsaturation = CVAR_INIT("gsaturation", "10", CV_SAVE | CV_CALL,
                                     saturation_cons_t, CV_palette_OnChange);
consvar_t cv_csaturation = CVAR_INIT("csaturation", "10", CV_SAVE | CV_CALL,
                                     saturation_cons_t, CV_palette_OnChange);
consvar_t cv_bsaturation = CVAR_INIT("bsaturation", "10", CV_SAVE | CV_CALL,
                                     saturation_cons_t, CV_palette_OnChange);
consvar_t cv_msaturation = CVAR_INIT("msaturation", "10", CV_SAVE | CV_CALL,
                                     saturation_cons_t, CV_palette_OnChange);

static CV_PossibleValue_t constextsize_cons_t[] = {
    {V_NOSCALEPATCH, "Small"},
    {V_SMALLSCALEPATCH, "Medium"},
    {V_MEDSCALEPATCH, "Large"},
    {0, "Huge"},
    {0, NULL}};
static void CV_constextsize_OnChange(void);
consvar_t cv_constextsize =
    CVAR_INIT("con_textsize", "Medium", CV_SAVE | CV_CALL, constextsize_cons_t,
              CV_constextsize_OnChange);
...
/*
The following was an extremely helpful resource when developing my Colour Cube
LUT. http://http.developer.nvidia.com/GPUGems2/gpugems2_chapter24.html Please
check it out if you're trying to maintain this. toast 18/04/17
*/
...
static boolean InitCube(void) {
  ...
  UINT8 q;
  float working[2][2][2]
               [3] = // the initial positions of the corners of the colour cube!
      {{{
...
#define gammascale 8
  globalgammamul =
      (cv_globalgamma.value
           ? ((255 - (gammascale * abs(cv_globalgamma.value))) / 255.0)
           : 1.0);
  globalgammaoffs = ((cv_globalgamma.value > 0)
                         ? ((gammascale * cv_globalgamma.value) / 255.0)
                         : 0.0);
  desatur[0] = desatur[1] = desatur[2] =
      globalgammaoffs + (0.33 * globalgammamul);

  if (doinggamma || diffcons(cv_rhue) || diffcons(cv_yhue) ||
      diffcons(cv_ghue) || diffcons(cv_chue) || diffcons(cv_bhue) ||
      diffcons(cv_mhue) || diffcons(cv_rgamma) || diffcons(cv_ygamma) ||
      diffcons(cv_ggamma) || diffcons(cv_cgamma) || diffcons(cv_bgamma) ||
      diffcons(cv_mgamma)) // set the gamma'd/hued positions (saturation is done
                           // later)
  {
    ...
    working[0][0][0][0] = working[0][0][0][1] = working[0][0][0][2] =
        globalgammaoffs;
    working[1][1][1][0] = working[1][1][1][1] = working[1][1][1][2] =
        globalgammaoffs + globalgammamul;

#define dohue(hue, gamma, loc)                                                 \
  tempgammamul =                                                               \
      (gamma ? ((255 - (gammascale * abs(gamma))) / 255.0) * globalgammamul    \
             : globalgammamul);                                                \
  tempgammaoffs =                                                              \
      ((gamma > 0) ? ((gammascale * gamma) / 255.0) + globalgammaoffs          \
                   : globalgammaoffs);                                         \
  ...
  if diffcons (cv_sat) {                                                       \
    ...
    tempgammaoffs =                                                            \
        ((gamma > 0) ? ((gammascale * gamma) / 255.0) + globalgammaoffs        \
                     : globalgammaoffs);                                       \
    for (q = 0; q < 3; q++)                                                    \
      dosaturation(working[r][g][b][q],                                        \
                   (tempgammaoffs + (desatur[q] * mod * tempgammamul)));       \
  }
...
// keep a copy of the palette so that we can get the RGB value for a color index
// at any time.
static void LoadPalette(const char *lumpname) {
  ...
  for (i = 0; i < palsize; i++) {
    ...
    pMasterPalette[i].s.green = pLocalPalette[i].s.green =
        correctiontable[*pal++];
    pMasterPalette[i].s.blue = pLocalPalette[i].s.blue =
        correctiontable[*pal++];
#else
    ...
    if (Cubeapply)
      V_CubeApply(&pLocalPalette[i].s.red, &pLocalPalette[i].s.green,
                  &pLocalPalette[i].s.blue);
  }
}
...
static void LoadMapPalette(void) { LoadPalette(GetPalette()); }
...
// --------------------------------------------------------------------------
void VID_BlitLinearScreen(const UINT8 *srcptr, UINT8 *destptr, INT32 width,
                          INT32 height, size_t srcrowbytes,
                          size_t destrowbytes) {
...
static inline UINT8 standardpdraw(const UINT8 *dest, const UINT8 *source,
                                  fixed_t ofs) {
  (void)dest;
  return source[ofs >> FRACBITS];
}
static inline UINT8 mappedpdraw(const UINT8 *dest, const UINT8 *source,
                                fixed_t ofs) {
  (void)dest;
  return *(v_colormap + source[ofs >> FRACBITS]);
}
static inline UINT8 translucentpdraw(const UINT8 *dest, const UINT8 *source,
                                     fixed_t ofs) {
  return *(v_translevel + ((source[ofs >> FRACBITS] << 8) & 0xff00) +
           (*dest & 0xff));
}
static inline UINT8 transmappedpdraw(const UINT8 *dest, const UINT8 *source,
                                     fixed_t ofs) {
  return *(v_translevel +
           (((*(v_colormap + source[ofs >> FRACBITS])) << 8) & 0xff00) +
           (*dest & 0xff));
}

// Draws a patch scaled to arbitrary size.
void V_DrawStretchyFixedPatch(fixed_t x, fixed_t y, fixed_t pscale,
                              fixed_t vscale, INT32 scrn, patch_t *patch,
...

  {
    ...
    if (scrn & V_FLIP)
      offsetx =
          FixedMul((patch->width - patch->leftoffset) << FRACBITS, pscale) + 1;
    else
  ...
  if (splitscreen && (scrn & V_PERPLAYER)) {
    fixed_t adjusty = ((scrn & V_NOSCALESTART) ? vid.height : BASEVIDHEIGHT)
                      << (FRACBITS - 1);
    vdup >>= 1;
    rowfrac <<= 1;
    y >>= 1;
  ...
  if (scrn & V_NOSCALESTART) {
    ...
    if (!(scrn & V_SCALEPATCHMASK)) {
      // if it's meant to cover the whole screen, black out the rest (ONLY IF
      // TOP LEFT ISN'T TRANSPARENT)
      if (!v_translevel && x == 0 && patch->width == BASEVIDWIDTH && y == 0 &&
          patch->height == BASEVIDHEIGHT) {
  ...
  for (col = 0; (col >> FRACBITS) < patch->width;
       col += colfrac, ++offx, desttop++) {
    if (scrn & V_FLIP) // offx is measured from right edge instead of left
    {
      if (x + pwidth - offx <
          0) // don't draw off the left of the screen (WRAP PREVENTION)
        break;
      if (x + pwidth - offx >=
          vid.width) // don't draw off the right of the screen (WRAP PREVENTION)
        continue;
    } else {
      if (x + offx <
          0) // don't draw off the left of the screen (WRAP PREVENTION)
    ...
    for (unsigned i = 0; i < column->num_posts; i++) {
      ...
      for (ofs = 0; dest < deststop && (size_t)(ofs >> FRACBITS) < post->length;
           ofs += rowfrac) {
        if (dest >=
            screens[scrn & V_PARAMMASK]) // don't draw off the top of the screen
                                         // (CRASH PREVENTION)
...
void V_DrawCroppedPatch(fixed_t x, fixed_t y, fixed_t pscale, fixed_t vscale,
                        INT32 scrn, patch_t *patch, const UINT8 *colormap,
                        fixed_t sx, fixed_t sy, fixed_t w, fixed_t h) {
  UINT8 (*patchdrawfunc)(const UINT8 *, const UINT8 *, fixed_t);
  UINT32 alphalevel = ((scrn & V_ALPHAMASK) >> V_ALPHASHIFT);
  UINT32 blendmode = ((scrn & V_BLENDMASK) >> V_BLENDSHIFT);
  // boolean flip = false;
...
#ifdef HWRENDER
  // if (rendermode != render_soft && !con_startup)		// Not this
  // again
  if (rendermode == render_opengl) {
    HWR_DrawCroppedPatch(patch, x, y, pscale, vscale, scrn, colormap, sx, sy, w,
                         h);
    return;
  }
#endif
...
#error Auto-cropping doesnt take quadscreen into account! Fix it!
      // Hint: For player 1/2, copy player 1's code below. For player 3/4, copy
      // player 2's code below For player 1/3 and 2/4, hijack the X wrap
      // prevention lines? That's probably easiest
    } else
#endif
    // 2 players
    {
      if (stplyr ==
          &players[displayplayer]) // Player 1's screen, crop at the bottom
      {
        // Just put a big old stop sign halfway through the screen
        deststop -= vid.rowbytes * (vid.height >> 1);
      } else // if (stplyr == &players[secondarydisplayplayer]) // Player 2's
             // screen, crop at the top
      {
        if (y < (vid.height >> 1)) // If the top is above the border
        {
          sy += ((vid.height >> 1) - y) *
                rowfrac; // Start further down on the patch
          h -= ((vid.height >> 1) - y) *
               rowfrac; // Draw less downwards from the start
          desttop += ((vid.height >> 1) - y) *
                     vid.width; // Start drawing at the border
        }
      }
    }
  }

  for (col = sx; (col >> FRACBITS) < patch->width && (col - sx) < w;
       col += colfrac, ++x, desttop++) {
    ...
    for (unsigned i = 0; i < column->num_posts; i++) {
      ...
      if ((topdelta << FRACBITS) - sy > 0) {
        dest +=
            FixedInt(FixedMul((topdelta << FRACBITS) - sy, vdup)) * vid.width;
        ofs = 0;
      } else
        ofs = sy - (topdelta << FRACBITS);

      for (; dest < deststop && (size_t)(ofs >> FRACBITS) < post->length &&
             ((ofs - sy) + (topdelta << FRACBITS)) < h;
           ofs += rowfrac) {
        if (dest >=
            screens[scrn & V_PARAMMASK]) // don't draw off the top of the screen
                                         // (CRASH PREVENTION)
...
//
void V_DrawContinueIcon(INT32 x, INT32 y, INT32 flags, INT32 skinnum,
                        UINT16 skincolor) {
  if (skinnum >= 0 && skinnum < numskins &&
      skins[skinnum]->sprites[SPR2_XTRA].numframes > XTRA_CONTINUE) {
    ...
    const UINT8 *colormap =
        R_GetTranslationColormap(skinnum, skincolor, GTC_CACHE);

    V_DrawMappedPatch(x, y, flags, patch, colormap);
  } else
    V_DrawScaledPatch(x - 10, y - 14, flags,
                      W_CachePatchName("CONTINS", PU_PATCH));
}
...
//
void V_DrawBlock(INT32 x, INT32 y, INT32 scrn, INT32 width, INT32 height,
                 const UINT8 *src) {
  UINT8 *dest;
...
void V_DrawFill(INT32 x, INT32 y, INT32 w, INT32 h, INT32 c) {
  ...
  if (splitscreen && (c & V_PERPLAYER)) {
    ...
    {
      fixed_t adjustx =
          ((c & V_NOSCALESTART) ? vid.height : BASEVIDHEIGHT) >> 1;
      w >>= 1;
      x >>= 1;
      if (stplyr == &players[displayplayer]) {
  ...
  if (!(c & V_NOSCALESTART)) {
    if (x == 0 && y == 0 && w == BASEVIDWIDTH &&
        h == BASEVIDHEIGHT) { // Clear the entire screen, from dest to deststop.
...
#ifdef HWRENDER
// This is now a function since it's otherwise repeated 2 times and honestly
// looks retarded:
...
void V_DrawFillConsoleMap(INT32 x, INT32 y, INT32 w, INT32 h, INT32 c) {
  ...
  if (rendermode == render_opengl) {
    ...
    HWR_DrawConsoleFill(
        x, y, w, h, c,
        hwcolor); // we still use the regular color stuff but only for flags.
                  // actual draw color is "hwcolor" for this.
    return;
  }
#endif
...
//
void V_DrawFadeFill(INT32 x, INT32 y, INT32 w, INT32 h, INT32 c, UINT16 color,
                    UINT8 strength) {
  UINT8 *dest;
  const UINT8 *deststop;
  INT32 u;
...
#ifdef HWRENDER
  if (rendermode == render_opengl) {
    // ughhhhh please can someone else do this? thanks ~toast 25/7/19 in 38
    // degrees centigrade w/o AC
    HWR_DrawFadeFill(x, y, w, h, c, color,
                     strength); // toast two days later - left above comment in
                                // 'cause it's funny
    return;
  }
#endif
  ...
  fadetable = ((color & 0xFF00) // Color is not palette index?
                   ...
                   : ((UINT8 *)R_GetTranslucencyTable((9 - strength) + 1) +
                      color * 256)); // Else, do TRANSMAP** fade.
  for (; (--h >= 0) && dest < deststop; dest += vid.width) {
    u = 0;
    while (u < w) {
...
//
// Fills a box of pixels using a flat texture as a pattern, scaled to screen
// size.
//
...

  {
    const UINT8 *fadetable =
        ((color & 0xFF00) // Color is not palette index?
             ? ((UINT8 *)(((color & 0x0F00) == 0x0A00)
                              ? fadecolormap // Do fadecolormap fade.
                              : (((color & 0x0F00) == 0x0B00)
                                     ? fadecolormap +
                                           (256 *
                                            FADECOLORMAPROWS) // Do white
                                                              // fadecolormap
                                                              // fade.
                                     : colormaps)) +
                strength * 256) // Do COLORMAP fade.
             : ((UINT8 *)R_GetTranslucencyTable((9 - strength) + 1) +
                color * 256)); // Else, do TRANSMAP** fade.
...
// Simple translucency with one color, over a set number of lines starting from
// the top.
...
// Very similar to F_DrawFadeConsBack, except we draw from the middle(-ish) of
...
void V_DrawPromptBack(INT32 boxheight, INT32 color) {
  ...
  if (color >= 256 && color < 512) {
    ...
    V_DrawFill((BASEVIDWIDTH - (vid.width / vid.dup)) / 2,
               BASEVIDHEIGHT - boxheight, (vid.width / vid.dup), boxheight,
               (color - 256) | V_SNAPTOBOTTOM);
    return;
  }
  ...
  if (rendermode == render_opengl) {
    ...
    case 1:
      hwcolor = 0x00000000;
      break; // Black // Note this is different from V_DrawFadeConsBack
    case 2:
      hwcolor = 0xdeb88700;
      break; // Sepia
    case 3:
...
// Generalized character mapping function for all text drawers.
// Handles uppercase conversion and Spanish fallback mapping.
static INT32 V_GetFontCharIdx(INT32 c, fontdef_t font,
                              boolean lowercaseallowed) {
  c &= 0xff;

  if (!lowercaseallowed && c < 128)
    c = toupper(c);

  // Fallback mapping for Spanish characters if sprites don't exist
  if (c >= FONTSTART && (c - FONTSTART) < FONTSIZE &&
      !font.chars[c - FONTSTART]) {
    int original = c;
    switch (original) {
    case 225:
    case 193:
    ...
    if (c != original) {
      if (lowercaseallowed && c >= 'A' && c <= 'Z') {
        int low = c + 32;
        if (low - FONTSTART >= 0 && low - FONTSTART < FONTSIZE &&
            font.chars[low - FONTSTART])
          c = low;
      }
      if (!lowercaseallowed)
        c = toupper(c);
...
// Procedural Spanish accent drawing helper.
// Overlays accent marks on base characters.
static void V_DrawSpanishAccent(fixed_t x, fixed_t y, fixed_t pscale,
                                fixed_t vscale, INT32 flags, INT32 original_c,
                                fontdef_t font, const UINT8 *colormap) {
  INT32 accent_c = 0;
  fixed_t yoffs = -FixedMul(3 << FRACBITS, vscale);

  switch (original_c & 0xff) {
  case 225:
  case 193: // á, Á
  case 233:
  case 201: // é, É
  ...
  case 209: // ñ, Ñ
    accent_c = '~';
    yoffs = -FixedMul(2 << FRACBITS, vscale);
    break;
  case 252:
  case 220: // ü, Ü
    accent_c = '"';
  ...
  if (accent_c) {
    INT32 acc_idx = accent_c - FONTSTART;
    if (acc_idx >= 0 && acc_idx < FONTSIZE && font.chars[acc_idx]) {
      V_DrawStretchyFixedPatch(x, y + yoffs, pscale, vscale, flags,
                               font.chars[acc_idx], colormap);
    }
  }
}

// Generalized character drawing function, combining console & chat
// functionality with a specified font.
//
void V_DrawFontCharacter(INT32 x, INT32 y, INT32 c, boolean lowercaseallowed,
                         fixed_t scale, UINT8 *colormap, fontdef_t font) {
  INT32 w, flags;
  INT32 original_c = c;
  const UINT8 *color = colormap ? colormap : V_GetStringColormap(c);

  flags = c & ~(V_CHARCOLORMASK | V_PARAMMASK);
  c = V_GetFontCharIdx(c, font, lowercaseallowed);

  if (c < 0 || c >= FONTSIZE || !font.chars[c])
    return;

  w = FixedMul(font.chars[c]->width / 2,
               scale); // use normal sized characters if we're using a terribly
                       // low resolution.
  if (x + w > vid.width)
    return;

  V_DrawFixedPatch(x * FRACUNIT, y * FRACUNIT, scale, flags, font.chars[c],
                   color);

  // Procedural Accent Pass
  V_DrawSpanishAccent(x * FRACUNIT, y * FRACUNIT, scale, scale, flags,
                      original_c, font, color);
}

// Precompile a wordwrapped string to any given width, using a specified font.
//
char *V_FontWordWrap(INT32 x, INT32 w, INT32 option, fixed_t scale,
  ...
  for (i = 0; i < slen; ++i) {
    c = newstring[i];
    if ((UINT8)c >= 0x80 && (UINT8)c <= 0x8F) // color parsing!
      continue;
    ...
    c = V_GetFontCharIdx(c, font, (option & V_ALLOWLOWERCASE));
    if (c < 0 || c >= FONTSIZE || !font.chars[c]) {
      chw = spacewidth;
      lastusablespace = i;
    } else
...
void V_DrawFontString(INT32 x, INT32 y, INT32 option, fixed_t pscale,
                      fixed_t vscale, const char *string, fontdef_t font) {
  V_DrawFontStringAtFixed((fixed_t)x << FRACBITS, (fixed_t)y << FRACBITS,
                          option, pscale, vscale, string, font);
}

void V_DrawAlignedFontString(INT32 x, INT32 y, INT32 option, fixed_t pscale,
                             fixed_t vscale, const char *string, fontdef_t font,
                             enum string_align align) {
  V_DrawAlignedFontStringAtFixed((fixed_t)x << FRACBITS, (fixed_t)y << FRACBITS,
                                 option, pscale, vscale, string, font, align);
}

// Write a string, using a supplied font and scale, at fixed_t coordinates.
// NOTE: The text is centered for screens larger than the base width.
void V_DrawFontStringAtFixed(fixed_t x, fixed_t y, INT32 option, fixed_t pscale,
                             fixed_t vscale, const char *string,
                             fontdef_t font) {
  fixed_t cx = x, cy = y;
  ...
  switch (option &
          V_SPACINGMASK) // TODO: 2.3: drop support for these crusty flags
  {
  case V_MONOSPACE:
    spacewidth = font.charwidth;
    /* FALLTHRU */
  ...
  for (;; ch++) {
    if (!*ch)
      break;
    if ((UINT8)*ch >= 0x80 && (UINT8)*ch <= 0x8F) // color parsing
    {
      // manually set flags override color codes
      if (!(option & V_CHARCOLORMASK))
        charflags = ((*ch & 0x0f) << V_CHARCOLORSHIFT) & V_CHARCOLORMASK;
      continue;
    }
    if (*ch == '\n') {
      cx = x;
      cy += FixedMul(((option & V_RETURN8) ? 8 : font.linespacing) << FRACBITS,
                     dupy);
      continue;
    }

    c = V_GetFontCharIdx(*ch, font, lowercase);
    if (c < 0 || c >= FONTSIZE || !font.chars[c]) {
    ...
    V_DrawStretchyFixedPatch(cx + center, cy, pscale, vscale, option,
                             font.chars[c], V_GetStringColormap(charflags));

    // Procedural Accent Pass
    V_DrawSpanishAccent(cx + center, cy, pscale, vscale, option, (UINT8)*ch,
                        font, V_GetStringColormap(charflags));

    cx += w + (font.kerning << FRACBITS);
  }
}

void V_DrawAlignedFontStringAtFixed(fixed_t x, fixed_t y, INT32 option,
  ...
  while (line) {
    ...
    ly += FixedMul(((option & V_RETURN8) ? 8 : font.linespacing) << FRACBITS,
                   vscale);
...
void V_DrawPaddedTallNum(INT32 x, INT32 y, INT32 flags, INT32 num,
                         INT32 digits) {
  INT32 w = tallnum[0]->width;
...
void V_DrawLevelActNum(INT32 x, INT32 y, INT32 flags, UINT8 num) {
  ...
  while (num > 0) {
    if (num > 9) // if there are two digits, draw second digit first
      V_DrawScaledPatch(
          x + (V_LevelActNumWidth(num) - V_LevelActNumWidth(num % 10)), y,
          flags, ttlnum[num % 10]);
    else
...
static void V_DrawNameTagLine(INT32 x, INT32 y, INT32 option, fixed_t scale,
                              UINT8 *basecolormap, UINT8 *outlinecolormap,
                              const char *string) {
  fixed_t cx, cy, w;
  ...
  for (;; ch++) {
    ...
    w = FixedMul(((ntb_font.chars[c]->width) + ntb_font.kerning * dup) *
                     FRACUNIT,
                 scale);
...
// Looks familiar.
void V_DrawNameTag(INT32 x, INT32 y, INT32 option, fixed_t scale,
                   UINT8 *basecolormap, UINT8 *outlinecolormap,
                   const char *string) {
  ...
  if (option & V_CENTERNAMETAG) {
    ...
    y -= FixedInt(
        FixedMul(((lbreakheight / 2) * (ntlines - 1)) * FRACUNIT, scale));
  }

  // No line breaks?
  // Draw entire string
  if (!last_token)
    V_DrawNameTagLine(x, y, option, scale, basecolormap, outlinecolormap,
                      string);
...
INT32 V_FontStringWidth(const char *string, INT32 option, fontdef_t font) {
  ...
  for (size_t i = 0; i < strlen(string); i++) {
    ...
    if ((UINT8)string[i] >= 0x80 && (UINT8)string[i] <= 0x8F) // color parsing
      continue;

    c = V_GetFontCharIdx((UINT8)string[i], font, (option & V_ALLOWLOWERCASE));
    if (c < 0 || c >= FONTSIZE || !font.chars[c])
      w += spacewidth;
    else
...
void V_DoPostProcessor(INT32 view, postimg_t type, INT32 param) {
  ...
  if (type == postimg_water) {
    ...
    angle_t disStart =
        (((leveltime - 1) * 128) + (rendertimefrac / (FRACUNIT / 128))) &
        FINEMASK;
    INT32 newpix;
    INT32 sine;
    ...
    for (y = yoffset; y < yoffset + height; y++) {
      ...
      if (sine < 0) {
        M_Memcpy(&tmpscr[y * vid.width + newpix], &srcscr[y * vid.width],
                 vid.width - newpix);
        ...
        M_Memcpy(&tmpscr[y * vid.width + 0], &srcscr[y * vid.width + sine],
                 vid.width - newpix);

        // Cleanup edge
        while (newpix) {
          tmpscr[y * vid.width + vid.width - newpix] =
              srcscr[y * vid.width + (vid.width - 1)];
          newpix--;
        }
      }
      ...
      srcscr[y*vid.width+newpix]; // *(transme + (srcscr[y*vid.width+x]<<8) +
      srcscr[y*vid.width+newpix]);
                              }*/
      disStart +=
          22; // the offset into the displacement map, increment each game loop
      disStart &= FINEMASK; // clip it to FINEMASK
    }

    VID_BlitLinearScreen(tmpscr + vid.width * vid.bpp * yoffset,
                         screens[0] + vid.width * vid.bpp * yoffset,
                         vid.width * vid.bpp, height, vid.width * vid.bpp,
                         vid.width);
  } else if (type == postimg_motion) // Motion Blur!
  {
    ...
    for (y = yoffset; y < yoffset + height; y++) {
      for (x = 0; x < vid.width; x++) {
        tmpscr[y * vid.width + x] =
            colormaps[*(transme + (srcscr[y * vid.width + x] << 8) +
                        (tmpscr[y * vid.width + x]))];
      }
    }
    VID_BlitLinearScreen(tmpscr + vid.width * vid.bpp * yoffset,
  ...
  } else if (type == postimg_flip) // Flip the screen upside-down
  {
    ...
    for (y = yoffset, y2 = yoffset + height - 1; y < yoffset + height;
         y++, y2--)
  ...
  } else if (type == postimg_heat) // Heat wave
  {
    ...
    for (y = yoffset; y < yoffset + height; y++) {
      if (heatshifter[heatindex[view]++]) {
        ...
        M_Memcpy(&tmpscr[y * vid.width + vid.dup], &srcscr[y * vid.width],
                 vid.width - vid.dup);
      } else
    ...
    if (renderisnewtic) // This isn't interpolated... but how do you interpolate
                        // a one-pixel shift?
    {
      heatindex[view]++;
...
void V_Recalc(void) {
  ...
  if (((vid.width * FRACUNIT) / BASEVIDWIDTH) <
      ((vid.height * FRACUNIT) / BASEVIDHEIGHT)) {
