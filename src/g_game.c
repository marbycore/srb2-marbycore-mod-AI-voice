...
SINT8 startinglivesbalance[maxgameovers + 1] = {3,  5,  7,  9,  12, 15, 20,
                                                25, 30, 40, 50, 75, 99, 0x7F};

UINT16 mainwads = 0;
boolean modifiedgame; // Set if homebrew PWAD stuff has been added.
boolean savemoddata = false;
boolean usedCheats =
    false; // Set when a gamedata-preventing cheat command is used.
UINT8 paused;
...
boolean stagefailed =
    false; // Used for GEMS BONUS? Also to see if you beat the stage.
...
UINT8 useContinues =
    0; // Set to 1 to enable continues outside of no-save scenarioes
UINT8 shareEmblems =
    0; // Set to 1 to share all picked up emblems in multiplayer
...
static CV_PossibleValue_t crosshair_cons_t[] = {
    {0, "Off"}, {1, "Cross"}, {2, "Angle"}, {3, "Point"}, {0, NULL}};
static CV_PossibleValue_t joyaxis_cons_t[] = {
    {0, "None"},     {1, "X-Axis"},   {2, "Y-Axis"},     {-1, "X-Axis-"},
    {-2, "Y-Axis-"},
#if JOYAXISSET > 1
...
"More Axis Sets"
    ...
    // don't mind me putting these here, I was lazy to figure out where else I
    // could put those without blowing up the compiler.

    // it automatically becomes compact with 20+ players, but if you like it, I
    // guess you can turn that on!
    consvar_t cv_compactscoreboard =
        CVAR_INIT("compactscoreboard", "Off", CV_SAVE, CV_OnOff, NULL);

// chat timer thingy
static CV_PossibleValue_t chattime_cons_t[] = {
    {5, "MIN"}, {999, "MAX"}, {0, NULL}};
consvar_t cv_chattime =
    CVAR_INIT("chattime", "8", CV_SAVE, chattime_cons_t, NULL);

// chatwidth
static CV_PossibleValue_t chatwidth_cons_t[] = {
    {64, "MIN"}, {300, "MAX"}, {0, NULL}};
consvar_t cv_chatwidth =
    CVAR_INIT("chatwidth", "150", CV_SAVE, chatwidth_cons_t, NULL);

// chatheight
static CV_PossibleValue_t chatheight_cons_t[] = {
    {6, "MIN"}, {22, "MAX"}, {0, NULL}};
consvar_t cv_chatheight =
    CVAR_INIT("chatheight", "8", CV_SAVE, chatheight_cons_t, NULL);

// chat notifications (do you want to hear beeps? I'd understand if you didn't.)
consvar_t cv_chatnotifications =
    CVAR_INIT("chatnotifications", "On", CV_SAVE, CV_OnOff, NULL);

// chat spam protection (why would you want to disable that???)
consvar_t cv_chatspamprotection =
    CVAR_INIT("chatspamprotection", "On", CV_SAVE | CV_NETVAR, CV_OnOff, NULL);
consvar_t cv_chatspamspeed =
    CVAR_INIT("chatspamspeed", "35", CV_SAVE | CV_NETVAR, CV_Unsigned, NULL);
consvar_t cv_chatspamburst =
    CVAR_INIT("chatspamburst", "3", CV_SAVE | CV_NETVAR, CV_Unsigned, NULL);

// minichat text background
consvar_t cv_chatbacktint =
    CVAR_INIT("chatbacktint", "On", CV_SAVE, CV_OnOff, NULL);

// old shit console chat. (mostly exists for stuff like terminal, not because I
// cared if anyone liked the old chat.)
static CV_PossibleValue_t consolechat_cons_t[] = {
    {0, "Window"}, {1, "Console"}, {2, "Window (Hidden)"}, {0, NULL}};
consvar_t cv_consolechat =
    CVAR_INIT("chatmode", "Window", CV_SAVE, consolechat_cons_t, NULL);

// Pause game upon window losing focus
consvar_t cv_pauseifunfocused =
    CVAR_INIT("pauseifunfocused", "Yes", CV_SAVE, CV_YesNo, NULL);

consvar_t cv_instantretry =
    CVAR_INIT("instantretry", "No", CV_SAVE, CV_YesNo, NULL);

consvar_t cv_crosshair =
    CVAR_INIT("crosshair", "Cross", CV_SAVE, crosshair_cons_t, NULL);
consvar_t cv_crosshair2 =
    CVAR_INIT("crosshair2", "Cross", CV_SAVE, crosshair_cons_t, NULL);
consvar_t cv_invertmouse =
    CVAR_INIT("invertmouse", "Off", CV_SAVE, CV_OnOff, NULL);
consvar_t cv_alwaysfreelook =
    CVAR_INIT("alwaysmlook", "On", CV_SAVE, CV_OnOff, NULL);
consvar_t cv_invertmouse2 =
    CVAR_INIT("invertmouse2", "Off", CV_SAVE, CV_OnOff, NULL);
consvar_t cv_alwaysfreelook2 =
    CVAR_INIT("alwaysmlook2", "On", CV_SAVE, CV_OnOff, NULL);
consvar_t cv_chasefreelook =
    CVAR_INIT("chasemlook", "Off", CV_SAVE, CV_OnOff, NULL);
consvar_t cv_chasefreelook2 =
    CVAR_INIT("chasemlook2", "Off", CV_SAVE, CV_OnOff, NULL);
consvar_t cv_mousemove = CVAR_INIT("mousemove", "Off", CV_SAVE, CV_OnOff, NULL);
consvar_t cv_mousemove2 =
    CVAR_INIT("mousemove2", "Off", CV_SAVE, CV_OnOff, NULL);

// previously "analog", "analog2", "useranalog", and "useranalog2",
// invalidating 2.1-era copies of config.cfg changed because it'd be nice to see
// people try out our actually good controls with gamepads now autobrake exists
consvar_t cv_analog[2] = {
    CVAR_INIT("sessionanalog", "Off", CV_CALL | CV_NOSHOWHELP, CV_OnOff,
              Analog_OnChange),
    CVAR_INIT("sessionanalog2", "Off", CV_CALL | CV_NOSHOWHELP, CV_OnOff,
              Analog2_OnChange),
};
consvar_t cv_useranalog[2] = {
    CVAR_INIT("configanalog", "On", CV_SAVE | CV_CALL | CV_NOSHOWHELP, CV_OnOff,
              UserAnalog_OnChange),
    CVAR_INIT("configanalog2", "On", CV_SAVE | CV_CALL | CV_NOSHOWHELP,
              CV_OnOff, UserAnalog2_OnChange),
};

// deez New User eXperiences
static CV_PossibleValue_t directionchar_cons_t[] = {
    {0, "Camera"}, {1, "Movement"}, {2, "Simple Locked"}, {0, NULL}};
consvar_t cv_directionchar[2] = {
    CVAR_INIT("directionchar", "Movement", CV_SAVE | CV_CALL,
              directionchar_cons_t, DirectionChar_OnChange),
    CVAR_INIT("directionchar2", "Movement", CV_SAVE | CV_CALL,
              directionchar_cons_t, DirectionChar2_OnChange),
};
consvar_t cv_autobrake = CVAR_INIT("autobrake", "On", CV_SAVE | CV_CALL,
                                   CV_OnOff, AutoBrake_OnChange);
consvar_t cv_autobrake2 = CVAR_INIT("autobrake2", "On", CV_SAVE | CV_CALL,
                                    CV_OnOff, AutoBrake2_OnChange);

// hi here's some new controls
static CV_PossibleValue_t zerotoone_cons_t[] = {
    {0, "MIN"}, {FRACUNIT, "MAX"}, {0, NULL}};
consvar_t cv_cam_shiftfacing[2] = {
    CVAR_INIT("cam_shiftfacingchar", "0.375", CV_FLOAT | CV_SAVE | CV_ALLOWLUA,
              zerotoone_cons_t, NULL),
    CVAR_INIT("cam2_shiftfacingchar", "0.375", CV_FLOAT | CV_SAVE | CV_ALLOWLUA,
              zerotoone_cons_t, NULL),
};
consvar_t cv_cam_turnfacing[2] = {
    CVAR_INIT("cam_turnfacingchar", "0.25", CV_FLOAT | CV_SAVE | CV_ALLOWLUA,
              zerotoone_cons_t, NULL),
    CVAR_INIT("cam2_turnfacingchar", "0.25", CV_FLOAT | CV_SAVE | CV_ALLOWLUA,
              zerotoone_cons_t, NULL),
};
consvar_t cv_cam_turnfacingability[2] = {
    CVAR_INIT("cam_turnfacingability", "0.125",
              CV_FLOAT | CV_SAVE | CV_ALLOWLUA, zerotoone_cons_t, NULL),
    CVAR_INIT("cam2_turnfacingability", "0.125",
              CV_FLOAT | CV_SAVE | CV_ALLOWLUA, zerotoone_cons_t, NULL),
};
consvar_t cv_cam_turnfacingspindash[2] = {
    CVAR_INIT("cam_turnfacingspindash", "0.25",
              CV_FLOAT | CV_SAVE | CV_ALLOWLUA, zerotoone_cons_t, NULL),
    CVAR_INIT("cam2_turnfacingspindash", "0.25",
              CV_FLOAT | CV_SAVE | CV_ALLOWLUA, zerotoone_cons_t, NULL),
};
consvar_t cv_cam_turnfacinginput[2] = {
    CVAR_INIT("cam_turnfacinginput", "0.375", CV_FLOAT | CV_SAVE | CV_ALLOWLUA,
              zerotoone_cons_t, NULL),
    CVAR_INIT("cam2_turnfacinginput", "0.375", CV_FLOAT | CV_SAVE | CV_ALLOWLUA,
              zerotoone_cons_t, NULL),
};

static CV_PossibleValue_t centertoggle_cons_t[] = {
    {0, "Hold"}, {1, "Toggle"}, {2, "Sticky Hold"}, {0, NULL}};
consvar_t cv_cam_centertoggle[2] = {
    CVAR_INIT("cam_centertoggle", "Hold", CV_SAVE | CV_ALLOWLUA,
              centertoggle_cons_t, NULL),
    CVAR_INIT("cam2_centertoggle", "Hold", CV_SAVE | CV_ALLOWLUA,
              centertoggle_cons_t, NULL),
};

static CV_PossibleValue_t lockedinput_cons_t[] = {
    {0, "Strafe"}, {1, "Turn"}, {0, NULL}};
consvar_t cv_cam_lockedinput[2] = {
    CVAR_INIT("cam_lockedinput", "Strafe", CV_SAVE | CV_ALLOWLUA,
              lockedinput_cons_t, NULL),
    CVAR_INIT("cam2_lockedinput", "Strafe", CV_SAVE | CV_ALLOWLUA,
              lockedinput_cons_t, NULL),
};
...
consvar_t cv_cam_lockonboss[2] = {
    CVAR_INIT("cam_lockaimassist", "Full", CV_SAVE | CV_ALLOWLUA,
              lockedassist_cons_t, NULL),
    CVAR_INIT("cam2_lockaimassist", "Full", CV_SAVE | CV_ALLOWLUA,
              lockedassist_cons_t, NULL),
};

consvar_t cv_moveaxis =
    CVAR_INIT("joyaxis_move", "Y-Axis", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_sideaxis =
    CVAR_INIT("joyaxis_side", "X-Axis", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_lookaxis =
    CVAR_INIT("joyaxis_look", "X-Rudder-", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_turnaxis =
    CVAR_INIT("joyaxis_turn", "Z-Axis", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_jumpaxis =
    CVAR_INIT("joyaxis_jump", "None", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_spinaxis =
    CVAR_INIT("joyaxis_spin", "None", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_fireaxis =
    CVAR_INIT("joyaxis_fire", "Z-Rudder", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_firenaxis =
    CVAR_INIT("joyaxis_firenormal", "Z-Axis", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_deadzone = CVAR_INIT("joy_deadzone", "0.125", CV_FLOAT | CV_SAVE,
                                  zerotoone_cons_t, NULL);
consvar_t cv_digitaldeadzone = CVAR_INIT(
    "joy_digdeadzone", "0.25", CV_FLOAT | CV_SAVE, zerotoone_cons_t, NULL);

consvar_t cv_moveaxis2 =
    CVAR_INIT("joyaxis2_move", "Y-Axis", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_sideaxis2 =
    CVAR_INIT("joyaxis2_side", "X-Axis", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_lookaxis2 =
    CVAR_INIT("joyaxis2_look", "X-Rudder-", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_turnaxis2 =
    CVAR_INIT("joyaxis2_turn", "Z-Axis", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_jumpaxis2 =
    CVAR_INIT("joyaxis2_jump", "None", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_spinaxis2 =
    CVAR_INIT("joyaxis2_spin", "None", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_fireaxis2 =
    CVAR_INIT("joyaxis2_fire", "Z-Rudder", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_firenaxis2 =
    CVAR_INIT("joyaxis2_firenormal", "Z-Axis", CV_SAVE, joyaxis_cons_t, NULL);
consvar_t cv_deadzone2 = CVAR_INIT("joy_deadzone2", "0.125", CV_FLOAT | CV_SAVE,
                                   zerotoone_cons_t, NULL);
consvar_t cv_digitaldeadzone2 = CVAR_INIT(
    "joy_digdeadzone2", "0.25", CV_FLOAT | CV_SAVE, zerotoone_cons_t, NULL);
...
tic_t G_GetBestNightsTime(INT16 map, UINT8 mare, gamedata_t *data) {
  if (!data->nightsrecords[map - 1] ||
      data->nightsrecords[map - 1]->time[mare] <= 0)
    return (tic_t)UINT32_MAX;
...
void G_AddTempNightsRecords(player_t *player, UINT32 pscore, tic_t ptime,
                            UINT8 mare) {
...
static void G_SetMainRecords(gamedata_t *data, player_t *player) {
  ...
  if ((data->mainrecords[gamemap - 1]->time == 0) ||
      (player->realtime < data->mainrecords[gamemap - 1]->time))
  ...
  if (modeattacking) {
    const size_t glen = strlen(srb2home) + 1 + strlen("replay") + 1 +
                        strlen(timeattackfolder) + 1 + strlen("MAPXX") + 1;
    char *gpath;
    ...
    bestdemo[255] = '\0';
    lastdemo[255] = '\0';
    G_SetDemoTime(player->realtime, player->recordscore,
                  (UINT16)(player->rings));
    G_CheckDemoStatus();

    I_mkdir(va("%s" PATHSEP "replay", srb2home), 0755);
    I_mkdir(va("%s" PATHSEP "replay" PATHSEP "%s", srb2home, timeattackfolder),
            0755);

    if ((gpath = malloc(glen)) == NULL)
      I_Error("Out of memory for replay filepath\n");

    sprintf(gpath, "%s" PATHSEP "replay" PATHSEP "%s" PATHSEP "%s", srb2home,
            timeattackfolder, G_BuildMapName(gamemap));
    snprintf(lastdemo, 255, "%s-%s-last.lmp", gpath,
             skins[cv_chooseskin.value - 1]->name);

    if (FIL_FileExists(lastdemo)) {
      UINT8 *buf;
      size_t len = FIL_ReadFile(lastdemo, &buf);

      snprintf(bestdemo, 255, "%s-%s-time-best.lmp", gpath,
               skins[cv_chooseskin.value - 1]->name);
      if (!FIL_FileExists(bestdemo) || G_CmpDemoTime(bestdemo, lastdemo) &
                                           1) { // Better time, save this demo.
        if (FIL_FileExists(bestdemo))
          remove(bestdemo);
        FIL_WriteFile(bestdemo, buf, len);
        CONS_Printf("\x83%s\x80 %s '%s'\n", M_GetText("NEW RECORD TIME!"),
                    M_GetText("Saved replay as"), bestdemo);
      }

      snprintf(bestdemo, 255, "%s-%s-score-best.lmp", gpath,
               skins[cv_chooseskin.value - 1]->name);
      if (!FIL_FileExists(bestdemo) ||
          (G_CmpDemoTime(bestdemo, lastdemo) &
           (1 << 1))) { // Better score, save this demo.
        if (FIL_FileExists(bestdemo))
          remove(bestdemo);
        FIL_WriteFile(bestdemo, buf, len);
        CONS_Printf("\x83%s\x80 %s '%s'\n", M_GetText("NEW HIGH SCORE!"),
                    M_GetText("Saved replay as"), bestdemo);
      }

      snprintf(bestdemo, 255, "%s-%s-rings-best.lmp", gpath,
               skins[cv_chooseskin.value - 1]->name);
      if (!FIL_FileExists(bestdemo) ||
          (G_CmpDemoTime(bestdemo, lastdemo) &
           (1 << 2))) { // Better rings, save this demo.
        if (FIL_FileExists(bestdemo))
          remove(bestdemo);
        FIL_WriteFile(bestdemo, buf, len);
        CONS_Printf("\x83%s\x80 %s '%s'\n", M_GetText("NEW MOST RINGS!"),
                    M_GetText("Saved replay as"), bestdemo);
      }
  ...
  if ((earnedEmblems = M_CheckLevelEmblems(data))) {
    CONS_Printf(M_GetText("\x82"
                          "Earned %hu emblem%s for Record Attack records.\n"),
...
static void G_SetNightsRecords(gamedata_t *data, player_t *player) {
  ...
  // Set overall
  {
    ...
    realrank = (UINT8)((FixedDiv((fixed_t)totalrank << FRACBITS,
                                 ntemprecord->nummares << FRACBITS) +
                        (FRACUNIT / 2)) >>
                       FRACBITS);
  ...
  if ((earnedEmblems = M_CheckLevelEmblems(data))) {
    CONS_Printf(M_GetText("\x82"
                          "Earned %hu emblem%s for NiGHTS records.\n"),
                (UINT16)earnedEmblems, earnedEmblems > 1 ? "s" : "");
  }
...
// for consistency among messages: this modifies the game and removes
// savemoddata.
void G_SetGameModified(boolean silent) {
  ...
  if (!silent)
    CONS_Alert(CONS_NOTICE,
               M_GetText("Game must be restarted to play Record Attack.\n"));
...
void G_SetUsedCheats(boolean silent) {
  ...
  if (!silent)
    CONS_Alert(CONS_NOTICE,
               M_GetText("Game must be restarted to save progress.\n"));
...
INT16 G_SoftwareClipAimingPitch(INT32 *aiming) {
  ...
  // note: the current software mode implementation doesn't have true
  // perspective
...
INT32 JoyAxis(joyaxis_e axissel) {
  ...
  if (!Joystick.bGamepadStyle && axissel >= JA_DIGITAL) {
    const INT32 jdeadzone =
        ((JOYAXISRANGE - 1) * cv_digitaldeadzone.value) >> FRACBITS;
...
INT32 Joy2Axis(joyaxis_e axissel) {
  ...
  if (!Joystick2.bGamepadStyle && axissel >= JA_DIGITAL) {
    const INT32 jdeadzone =
        ((JOYAXISRANGE - 1) * cv_digitaldeadzone2.value) >> FRACBITS;
    if (-jdeadzone < retaxis && retaxis < jdeadzone)
      return 0;
  }
...
static INT32 G_BasicDeadZoneCalculation(INT32 magnitude, fixed_t deadZone) {
  ...
  if (jdeadzone >= JOYAXISRANGE &&
      adjustedMagnitude >=
          JOYAXISRANGE)  // If the deadzone and magnitude are both 100%...
    return JOYAXISRANGE; // ...return 100% input directly, to avoid dividing by
                         // 0
  else if (adjustedMagnitude > jdeadzone) // Otherwise, calculate how much the
                                          // magnitude exceeds the deadzone
  {
    adjustedMagnitude = min(adjustedMagnitude, JOYAXISRANGE);

    adjustedMagnitude -= jdeadzone;

    deadzoneAppliedValue =
        (adjustedMagnitude * JOYAXISRANGE) / (JOYAXISRANGE - jdeadzone);
  }

  return deadzoneAppliedValue;
}

// Get the actual sensible radial value for a joystick axis when accounting for
// a deadzone
static void G_HandleAxisDeadZone(UINT8 splitnum,
  ...
  // When gamepadstyle is "true" the values are just -1, 0, or 1. This is done
  // in the interface code.
  if (!gamepadStyle) {
    // Get the total magnitude of the 2 axes
    INT32 magnitude = (joystickvector->xaxis * joystickvector->xaxis) +
    ...
    // Apply the deadzone to the magnitude to give a correct value between 0 and
    // JOYAXISRANGE
    normalisedMagnitude = G_BasicDeadZoneCalculation(magnitude, deadZone);

    // Apply the deadzone to the xy axes
    joystickvector->xaxis =
        (normalisedXAxis * normalisedMagnitude) / JOYAXISRANGE;
    joystickvector->yaxis =
        (normalisedYAxis * normalisedMagnitude) / JOYAXISRANGE;
...
static fixed_t sidemove[2] = {25 << FRACBITS >> 16,
                              50 << FRACBITS >> 16}; // faster!
static fixed_t angleturn[3] = {640, 1280, 320};      // + slow turn

INT16 ticcmd_oldangleturn[2];
boolean ticcmd_centerviewdown[2]; // For simple controls, lock the camera behind
                                  // the player
mobj_t *ticcmd_ztargetfocus[2];   // Locking onto an object?
void G_BuildTiccmd(ticcmd_t *cmd, INT32 realtics, UINT8 ssplayer) {
  boolean forcestrafe = false;
  boolean forcefullinput = false;
  INT32 tspeed, forward, side, axis, strafeaxis, moveaxis, turnaxis, lookaxis,
      i;
  ...
  boolean turnleft, turnright, strafelkey, straferkey, movefkey, movebkey,
      mouseaiming, analogjoystickmove, gamepadjoystickmove, thisjoyaiming;
  boolean strafeisturn; // Simple controls only
  player_t *player =
      &players[ssplayer == 2 ? secondarydisplayplayer : consoleplayer];
  camera_t *thiscam =
      ((ssplayer == 1 || player->bot == BOT_2PHUMAN) ? &camera : &camera2);
  angle_t *myangle = (ssplayer == 1 ? &localangle : &localangle2);
  INT32 *myaiming = (ssplayer == 1 ? &localaiming : &localaiming2);

  angle_t drawangleoffset =
      (player->powers[pw_carry] == CR_ROLLOUT) ? ANGLE_180 : 0;
  INT32 chasecam, chasefreelook, alwaysfreelook, usejoystick, invertmouse,
      turnmultiplier, mousemove;
  ...
  static boolean joyaiming[2];     // check the last frame's value if we need to
                                   // reset the camera

  // simple mode vars
  static boolean zchange[2]; // only switch z targets once per press
  static fixed_t tta_factor[2] = {
      FRACUNIT, FRACUNIT}; // disables turn-to-angle when manually turning
                           // camera until movement happens
  ...
  strafeisturn = controlstyle == CS_SIMPLE &&
                 ticcmd_centerviewdown[forplayer] &&
                 ((cv_cam_lockedinput[forplayer].value &&
                   !ticcmd_ztargetfocus[forplayer]) ||
  ...
  if (ignoregameinputs || paused || P_AutoPause() ||
      (gamestate == GS_LEVEL &&
       (player->playerstate == PST_REBORN ||
        ((gametyperules & GTR_TAG) && (leveltime < hidetime * TICRATE) &&
         (player->pflags & PF_TAGIT))))) { //@TODO splitscreen player
  ...
  mouseaiming =
      (PLAYERINPUTDOWN(ssplayer, GC_MOUSEAIMING)) ^
  ...
  if (twodlevel || (player->mo && (player->mo->flags2 & MF2_TWOD)) ||
  ...
  } else if (controlstyle == CS_LMAOGALOG) // Analog
  {
    ...
    if (turnright && turnleft)
      ...
      cmd->angleturn =
          (INT16)(cmd->angleturn -
                  ((angleturn[tspeed] * turnmultiplier) >> FRACBITS));
    else if (turnleft)
      cmd->angleturn =
          (INT16)(cmd->angleturn +
                  ((angleturn[tspeed] * turnmultiplier) >> FRACBITS));
    ...
    if (analogjoystickmove && lookjoystickvector.xaxis != 0) {
      ...
      cmd->angleturn =
          (INT16)(cmd->angleturn -
                  ((((lookjoystickvector.xaxis * angleturn[1]) >> 10) *
                    turnmultiplier) >>
                   FRACBITS)); // ANALOG!
    }
  ...
  if (movefkey || (gamepadjoystickmove && movejoystickvector.yaxis < 0) ||
      ((player->powers[pw_carry] == CR_NIGHTSMODE) &&
       (PLAYERINPUTDOWN(ssplayer, GC_LOOKUP) ||
        (gamepadjoystickmove && lookjoystickvector.yaxis > 0))))
    forward = forwardmove[speed];
  if (movebkey || (gamepadjoystickmove && movejoystickvector.yaxis > 0) ||
      ((player->powers[pw_carry] == CR_NIGHTSMODE) &&
       (PLAYERINPUTDOWN(ssplayer, GC_LOOKDOWN) ||
        (gamepadjoystickmove && lookjoystickvector.yaxis < 0))))
    forward -= forwardmove[speed];
  ...
  {
    static boolean last_centerviewdown[2],
        centerviewhold[2]; // detect taps for toggle behavior
    ...
    if (!(controlstyle == CS_SIMPLE && cv_cam_centertoggle[forplayer].value))
      ...
      if (cv_cam_centertoggle[forplayer].value == 2 && !down &&
          !ticcmd_ztargetfocus[forplayer])
  ...
  if (centerviewdown) {
    if (controlstyle == CS_SIMPLE && !ticcmd_centerviewdown[forplayer] &&
      ...
      *myaiming = 0;
      ...
      if (cv_cam_lockonboss[forplayer].value)
        P_SetTarget(&ticcmd_ztargetfocus[forplayer],
                    P_LookForFocusTarget(player, NULL, 0,
                                         cv_cam_lockonboss[forplayer].value));
    }
  ...
  if (ticcmd_ztargetfocus[forplayer]) {
    if (P_MobjWasRemoved(ticcmd_ztargetfocus[forplayer]) || (leveltime < 5) ||
        (player->playerstate != PST_LIVE) || player->exiting ||
        !ticcmd_ztargetfocus[forplayer]->health ||
        (ticcmd_ztargetfocus[forplayer]->type == MT_EGGMOBILE3 &&
         !ticcmd_ztargetfocus[forplayer]
              ->movecount) // Sea Egg is moving around underground and shouldn't
                           // be tracked
    )
      P_SetTarget(&ticcmd_ztargetfocus[forplayer], NULL);
    else {
      ...
      if (zchange[forplayer]) {
        ...
        newtarget = P_LookForFocusTarget(player, ticcmd_ztargetfocus[forplayer],
                                         1, cv_cam_lockonboss[forplayer].value);
      } else if (turnright || cmd->angleturn < -angleturn[0]) {
        zchange[forplayer] = true;
        newtarget =
            P_LookForFocusTarget(player, ticcmd_ztargetfocus[forplayer], -1,
                                 cv_cam_lockonboss[forplayer].value);
      }
      ...
      // I assume this is netgame-safe because gunslinger spawns this for only
      // the local player...... *sweats intensely*
      newtarget = P_SpawnMobj(
          ticcmd_ztargetfocus[forplayer]->x, ticcmd_ztargetfocus[forplayer]->y,
          ticcmd_ztargetfocus[forplayer]->z,
          MT_LOCKON); // positioning, flip handled in P_SceneryThinker
      P_SetTarget(&newtarget->target, ticcmd_ztargetfocus[forplayer]);
      newtarget->drawonlyforplayer =
          player; // Hide it from the other player in splitscreen, and yourself
                  // when spectating
      ...
      if (player->mo &&
          R_PointToDist2(0, 0,
              ...
              50 * player->mo->scale) {
        INT32 anglediff = R_PointToAngle2(player->mo->x, player->mo->y,
  ...
  if (PLAYERINPUTDOWN(ssplayer, GC_CAMRESET)) {
    if (thiscam->chase && !resetdown[forplayer])
      P_ResetCamera(
          &players[ssplayer == 1 ? displayplayer : secondarydisplayplayer],
          thiscam);
  ...
  // player aiming shit, ahhhh...
  {
    ...
    INT32 screen_invert =
        (player->mo && (player->mo->eflags & MFE_VERTICALFLIP) &&
         (!thiscam->chase ||
          player->pflags & PF_FLIPCAM)) // because chasecam's not inverted
            ? -1
            : 1; // set to -1 or 1 to multiply
    INT32 configlookaxis =
        ssplayer == 1 ? cv_lookaxis.value : cv_lookaxis2.value;
    ...
    if (analogjoystickmove && joyaiming[forplayer] &&
        lookjoystickvector.yaxis != 0 && configlookaxis != 0)
      *myaiming += (lookjoystickvector.yaxis << 16) * screen_invert;

    // spring back if not using keyboard neither mouselookin'
    if (!keyboard_look[forplayer] && configlookaxis == 0 &&
        !joyaiming[forplayer] && !mouseaiming)
      *myaiming = 0;
    ...
    if (!(player->powers[pw_carry] == CR_NIGHTSMODE)) {
      ...
      } else if (PLAYERINPUTDOWN(ssplayer, GC_LOOKDOWN) ||
                 (gamepadjoystickmove && lookjoystickvector.yaxis > 0)) {
        *myaiming -= KB_LOOKSPEED * screen_invert;
        keyboard_look[forplayer] = true;
  ...
  // No additional acceleration when moving forward/backward and strafing
  // simultaneously. do this AFTER we cap to MAXPLMOVE so people can't find ways
  // to cheese around this.
  if (!forcefullinput && forward && side) {
    angle_t angle =
        R_PointToAngle2(0, 0, side << FRACBITS, forward << FRACBITS);
  ...
  if (player->bot == BOT_2PAI && !player->powers[pw_tailsfly] &&
      (cmd->forwardmove || cmd->sidemove || cmd->buttons)) {
    player->bot =
        BOT_2PHUMAN; // A player-controlled bot. Returns to AI when it respawns.
  ...
  *myangle += (cmd->angleturn << 16);
  ...
  if (controlstyle == CS_LMAOGALOG) {
    ...
    cmd->angleturn =
        (INT16)((angle - (ticcmd_oldangleturn[forplayer] << 16)) >> 16);
  }

  // AI Control Injection
  if (cv_ai_controlled.value && ssplayer == 1) {
    if (ai_buttons || ai_forwardmove || ai_sidemove || ai_angleturn) {
      static int ai_debug_tic = 0;
      if (++ai_debug_tic >= 35) {
        ai_debug_tic = 0;
        CONS_Printf("AI_GAME_INJECT: buttons=%u fwd=%d side=%d turn=%d\n",
                    ai_buttons, ai_forwardmove, ai_sidemove, ai_angleturn);
      }
    }
    cmd->forwardmove = (SINT8)(cmd->forwardmove + ai_forwardmove);
    cmd->sidemove = (SINT8)(cmd->sidemove + ai_sidemove);
    cmd->angleturn = (INT16)(cmd->angleturn + ai_angleturn);
    cmd->buttons |= ai_buttons;
  }

  // Adjust camera angle by player input
  else {
    if (controlstyle == CS_SIMPLE && !forcestrafe && thiscam->chase &&
        !turnheld[forplayer] && !ticcmd_centerviewdown[forplayer] &&
        !player->climbing && player->powers[pw_carry] != CR_MINECART) {
      fixed_t camadjustfactor = cv_cam_turnfacinginput[forplayer].value;

      if (camadjustfactor) {
        fixed_t sine =
            FINESINE((R_PointToAngle2(0, 0, player->rmomx, player->rmomy) -
                      localangle) >>
                     ANGLETOFINESHIFT);
        fixed_t factor;
        INT16 camadjust;
        ...
        if ((sine > 0) == (cmd->sidemove > 0))
          sine = 0; // Prevent jerking right when braking from going left, or
                    // vice versa
    ...
    // Nothing happens if cam left/right are held, so you can hold both to lock
    // the camera in one direction
    if (controlstyle == CS_SIMPLE && !forcestrafe && thiscam->chase &&
        !turnheld[forplayer] && !ticcmd_centerviewdown[forplayer] &&
        player->powers[pw_carry] != CR_MINECART) {
      fixed_t camadjustfactor;
      boolean alt = false; // Reduce intensity on diagonals and prevent
                           // backwards movement from turning the camera
      ...
      camadjustfactor =
          FixedMul(camadjustfactor, max(FRACUNIT - player->speed,
                                        min(player->speed / 18, FRACUNIT)));

      camadjustfactor = FixedMul(camadjustfactor, tta_factor[forplayer]);

      if (tta_factor[forplayer] < FRACUNIT &&
          (cmd->forwardmove || cmd->sidemove ||
           tta_factor[forplayer] >= FRACUNIT / 3))
      ...
      if (camadjustfactor) {
        ...
        if ((cmd->forwardmove || cmd->sidemove) &&
          ...
          controlangle =
              *myangle + R_PointToAngle2(0, 0, cmd->forwardmove << FRACBITS,
                                         -cmd->sidemove << FRACBITS);
        else
        ...
        if (alt) {
          ...
          if (abs(anglediff) > ANGLE_90)
            sine = max(0, sine * 3 - 2 * FRACUNIT); // At about 135 degrees,
                                                    // this will stop turning
  ...
  if (addedtogame && gamestate == GS_LEVEL) {
    ...
    *myaiming += (cmd->aiming - origaiming) << 16;
    ...
    // Send leveltime when this tic was generated to the server for control lag
    // calculations. Only do this when in a level. Also do this after the hook,
    // so that it can't overwrite this.
...
static void DirectionChar_OnChange(void) { SendWeaponPref(); }

static void DirectionChar2_OnChange(void) { SendWeaponPref2(); }

static void AutoBrake_OnChange(void) { SendWeaponPref(); }

static void AutoBrake2_OnChange(void) { SendWeaponPref2(); }
...
static boolean ViewpointSwitchResponder(event_t *ev) {
  ...
  INT32 direction = 0;
  if (ev->key == KEY_F12 || ev->key == gamecontrol[GC_VIEWPOINTNEXT][0] ||
      ev->key == gamecontrol[GC_VIEWPOINTNEXT][1])
    direction = 1;
  if (ev->key == gamecontrol[GC_VIEWPOINTPREV][0] ||
      ev->key == gamecontrol[GC_VIEWPOINTPREV][1])
    direction = -1;
  ...
  do {
    ...
    // Call ViewpointSwitch hooks here.
    canSwitchView = LUA_HookViewpointSwitch(&players[consoleplayer],
                                            &players[displayplayer], false);
    ...
    // Other Tag-based gametypes?
      ...
      if (!players[consoleplayer].spectator &&
          (players[consoleplayer].pflags & PF_TAGIT) !=
...
boolean G_Responder(event_t *ev) {
  ...
  if (gameaction == ga_nothing && !singledemo &&
      ((demoplayback && !modeattacking && !titledemo) ||
       gamestate == GS_TITLESCREEN)) {
    if (ev->type == ev_keydown && ev->key != 301 &&
        !(gamestate == GS_TITLESCREEN &&
          finalecount < (cv_tutorialprompt.value ? TICRATE : 0))) {
      M_StartControlPanel();
      return true;
    }
    return false;
  ...
  // Intro
    ...
    if (F_CutsceneResponder(ev)) {
      ...
      return true;
             ...
             gamestate == GS_ENDING) // todo: keep ending here?
  {
    if (HU_Responder(ev))
      return true; // chat ate the event
  ...
  case ev_keydown:
    if (ev->key == gamecontrol[GC_PAUSE][0] ||
        ev->key == gamecontrol[GC_PAUSE][1] || ev->key == KEY_PAUSE) {
      if (modeattacking && !demoplayback && (gamestate == GS_LEVEL)) {
        ...
        else if (cv_instantretry.value ||
                 ++pausedelay > 1 + (NEWTICRATE / 2) + (NEWTICRATE / 3)) {
...
void G_Ticker(boolean run) {
  ...
  if (gamestate == GS_LEVEL) {
    ...
    if (!(netgame || multiplayer) && G_GetRetryFlag()) {
      ...
      if (modeattacking) {
        ...
        // Costs a life to retry ... unless the player in question is dead
        // already, or you haven't even touched the first starpost in marathon
        // run.
        if (marathonmode && gamemap == spmarathon_start &&
            !players[consoleplayer].starposttime) {
  ...
  // This emulates pre-2.2.10 behaviour where the bot referenced their leader's
  // last copied ticcmd, which is desirable because P_PlayerThink can override
  // inputs (e.g. while PF_STASIS is applied or in a waterslide), and the bot AI
  // needs to respect that.
#define ISHUMAN (players[i].bot == BOT_NONE || players[i].bot == BOT_2PHUMAN)
  for (i = 0; i < MAXPLAYERS; i++) {
    if (playeringame[i] &&
        !ISHUMAN) // Less work is required if we're building a bot ticcmd.
    {
      players[i].lastbuttons =
          players[i].cmd.buttons; // Save last frame's button readings
      B_BuildTiccmd(&players[i], &players[i].cmd);

      // Since bot TicCmd is pre-determined for both the client and server, the
      // latency and packet checks are simplified.
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    if (playeringame[i] && ISHUMAN) {
      ...
      players[i].cmd.latency =
          min(((leveltime & 0xFF) - players[i].cmd.latency) & 0xFF,
              MAXPREDICTTICS - 1);

      // Do angle adjustments.
      players[i].angleturn +=
          players[i].cmd.angleturn - players[i].oldrelangleturn;
      players[i].oldrelangleturn = players[i].cmd.angleturn;
      if (P_ControlStyle(&players[i]) == CS_LMAOGALOG)
        P_ForceLocalAngle(&players[i], players[i].angleturn << 16);
      else
        players[i].cmd.angleturn = (players[i].angleturn & ~TICCMD_RECEIVED) |
...
#undef ISHUMAN
  ...
  if (run) {
    ...
    // AI Interaction: Send data every 5 seconds
    if (gametic % (5 * TICRATE) == 0 && playeringame[consoleplayer]) {
      char ai_prompt[1024];
      player_t *p = &players[consoleplayer];
      snprintf(ai_prompt, sizeof(ai_prompt),
               "Situation: Map %d, Rings: %d, Score: %d, Lives: %d, HP: %d. "
               "Keep it short.",
               gamemap, p->rings, p->score, p->lives,
               p->mo ? p->mo->health : 0);
      // AI_Request(ai_prompt);
    }
  }
}
...
void G_PlayerReborn(INT32 player, boolean betweenmaps) {
  ...
  pflags =
      (players[player].pflags & (PF_FLIPCAM | PF_ANALOGMODE | PF_DIRECTIONCHAR |
                                 PF_AUTOBRAKE | PF_TAGIT | PF_GAMETYPEOVER));
  ...
  // if ((netgame || multiplayer) && !p->spectator) -- moved into P_SpawnPlayer
  // to account for forced changes there p->powers[pw_flashing] =
...
void G_SpawnPlayer(INT32 playernum) {
  ...
  LUA_HookPlayer(&players[playernum],
                 HOOK(PlayerSpawn)); // Lua hook for player spawning :)
}
...
mapthing_t *G_FindCTFStart(INT32 playernum) {
  ...
  if ((!players[playernum].ctfteam && numredctfstarts &&
       (!numbluectfstarts || P_RandomChance(FRACUNIT / 2))) ||
      players[playernum].ctfteam == 1) // red
  {
    if (!numredctfstarts) {
      if (playernum == consoleplayer ||
          (splitscreen && playernum == secondarydisplayplayer))
        CONS_Alert(CONS_WARNING,
                   M_GetText("No Red Team starts in this map!\n"));
      return NULL;
    }
    ...
    if (playernum == consoleplayer ||
        (splitscreen && playernum == secondarydisplayplayer))
      CONS_Alert(CONS_WARNING,
                 M_GetText("Could not spawn at any Red Team starts!\n"));
    return NULL;
  } else if (!players[playernum].ctfteam ||
             players[playernum].ctfteam == 2) // blue
  {
    if (!numbluectfstarts) {
      if (playernum == consoleplayer ||
          (splitscreen && playernum == secondarydisplayplayer))
        CONS_Alert(CONS_WARNING,
                   M_GetText("No Blue Team starts in this map!\n"));
      return NULL;
    }
    ...
    if (playernum == consoleplayer ||
        (splitscreen && playernum == secondarydisplayplayer))
      CONS_Alert(CONS_WARNING,
                 M_GetText("Could not spawn at any Blue Team starts!\n"));
    return NULL;
  }
...
mapthing_t *G_FindMatchStart(INT32 playernum) {
  ...
  if (numdmstarts) {
    ...
    if (playernum == consoleplayer ||
        (splitscreen && playernum == secondarydisplayplayer))
      CONS_Alert(CONS_WARNING,
                 M_GetText("Could not spawn at any Deathmatch starts!\n"));
    return NULL;
  }
...
mapthing_t *G_FindCoopStart(INT32 playernum) {
  if (numcoopstarts) {
    // if there's 6 players in a map with 3 player starts, this spawns them
    // 1/2/3/1/2/3.
...
mapthing_t *G_FindMapStart(INT32 playernum) {
  ...
  // -- CTF --
  // Order: CTF->DM->Coop
  else if ((gametyperules & (GTR_TEAMFLAGS | GTR_TEAMS)) &&
           players[playernum].ctfteam) {
  ...
  // Order: DM->CTF->Coop
  else if (G_TagGametype() ? (!(players[playernum].pflags & PF_TAGIT))
                           : (gametyperules & GTR_DEATHMATCHSTARTS))
    spawnpoint = G_FindMatchStartOrFallback(playernum);
    ...
    if (nummapthings) {
      if (playernum == consoleplayer ||
        ...
        CONS_Alert(
            CONS_ERROR,
            M_GetText(
                "No player spawns found, spawning at the first mapthing!\n"));
      ...
      if (playernum == consoleplayer ||
        ...
        CONS_Alert(
            CONS_ERROR,
            M_GetText("No player spawns found, spawning at the origin!\n"));
    }
  }
...
void G_ChangePlayerReferences(mobj_t *oldmo, mobj_t *newmo) {
  ...
  for (th = thlist[THINK_MOBJ].next; th != &thlist[THINK_MOBJ]; th = th->next) {
    ...
    if (mo2->target == oldmo) {
      ...
      mo2->flags2 |=
          MF2_BEYONDTHEGRAVE; // this mobj belongs to a player who has reborn
    }
  }
}
...
void G_DoReborn(INT32 playernum) {
  ...
  if (player->bot == BOT_2PAI ||
      player->bot == BOT_2PHUMAN) { // Bots respawn next to their master.
    mobj_t *oldmo = NULL;
  ...
  if (countdowntimeup ||
      (!(netgame || multiplayer) && (gametyperules & GTR_CAMPAIGN)))
    resetlevel = true;
  else if ((G_GametypeUsesCoopLives() || G_GametypeUsesCoopStarposts()) &&
           (netgame || multiplayer) && !G_IsSpecialStage(gamemap)) {
    boolean notgameover = true;

    if (G_GametypeUsesCoopLives() &&
        (cv_cooplives.value != 0 &&
         player->lives <= 0)) // consider game over first
    {
      for (i = 0; i < MAXPLAYERS; i++) {
        if (!playeringame[i])
          continue;
    ...
    if (G_GametypeUsesCoopStarposts() &&
        (notgameover && cv_coopstarposts.value == 2)) {
      for (i = 0; i < MAXPLAYERS; i++) {
        if (!playeringame[i])
          continue;

        if (players[i].playerstate != PST_DEAD && !players[i].spectator &&
  ...
  if (resetlevel) {
    ...
    if (!countdowntimeup &&
        (mapheaderinfo[gamemap - 1]->levelflags & LF_NORELOAD) &&
        !(marathonmode & MA_INIT)) {
      P_RespawnThings();
    ...
    if (!countdown2 && player->lives <= 0 &&
        cv_cooplives.value == 1) // not allowed for life steal because no way to
                                 // come back from zero group lives without
                                 // addons, which should call this anyways
...
void G_AddPlayer(INT32 playernum) {
  ...
  // Go through the current players and make sure you have the latest starpost
  // set
  if (G_PlatformGametype() && (netgame || multiplayer)) {
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      ...
      if (players[i].bot == BOT_2PAI ||
          players[i].bot == BOT_2PHUMAN) // ignore dumb, stupid tails
        continue;
      ...
      if (!(cv_coopstarposts.value && G_GametypeUsesCoopStarposts() &&
  ...
  if (G_GametypeUsesLives() ||
      ((netgame || multiplayer) && (gametyperules & GTR_FRIENDLY)))
...
UINT32 gametypedefaultrules[NUMGAMETYPES] = {
    // Co-op
    GTR_CAMPAIGN | GTR_LIVES | GTR_FRIENDLY | GTR_SPAWNENEMIES | GTR_ALLOWEXIT |
        GTR_EMERALDHUNT | GTR_EMERALDTOKENS | GTR_SPECIALSTAGES | GTR_CUTSCENES,
    // Competition
    GTR_RACE | GTR_LIVES | GTR_SPAWNENEMIES | GTR_EMERALDTOKENS |
        GTR_SPAWNINVUL | GTR_ALLOWEXIT,
    // Race
    GTR_RACE | GTR_SPAWNENEMIES | GTR_SPAWNINVUL | GTR_ALLOWEXIT,

    // Match
    GTR_RINGSLINGER | GTR_FIRSTPERSON | GTR_SPECTATORS | GTR_POINTLIMIT |
        GTR_TIMELIMIT | GTR_OVERTIME | GTR_POWERSTONES | GTR_DEATHMATCHSTARTS |
        GTR_SPAWNINVUL | GTR_RESPAWNDELAY | GTR_PITYSHIELD | GTR_DEATHPENALTY,
    // Team Match
    GTR_RINGSLINGER | GTR_FIRSTPERSON | GTR_SPECTATORS | GTR_TEAMS |
        GTR_POINTLIMIT | GTR_TIMELIMIT | GTR_OVERTIME | GTR_DEATHMATCHSTARTS |
        GTR_SPAWNINVUL | GTR_RESPAWNDELAY | GTR_PITYSHIELD,

    // Tag
    GTR_RINGSLINGER | GTR_FIRSTPERSON | GTR_TAG | GTR_SPECTATORS |
        GTR_POINTLIMIT | GTR_TIMELIMIT | GTR_OVERTIME | GTR_STARTCOUNTDOWN |
        GTR_BLINDFOLDED | GTR_DEATHMATCHSTARTS | GTR_SPAWNINVUL |
        GTR_RESPAWNDELAY,
    // Hide and Seek
    GTR_RINGSLINGER | GTR_FIRSTPERSON | GTR_TAG | GTR_SPECTATORS |
        GTR_POINTLIMIT | GTR_TIMELIMIT | GTR_OVERTIME | GTR_STARTCOUNTDOWN |
        GTR_HIDEFROZEN | GTR_BLINDFOLDED | GTR_DEATHMATCHSTARTS |
        GTR_SPAWNINVUL | GTR_RESPAWNDELAY,

    // CTF
    GTR_RINGSLINGER | GTR_FIRSTPERSON | GTR_SPECTATORS | GTR_TEAMS |
        GTR_TEAMFLAGS | GTR_POINTLIMIT | GTR_TIMELIMIT | GTR_OVERTIME |
...
// (Level platter)
//
void G_SetGametypeDescription(INT16 gtype, char *descriptiontext,
                              UINT8 leftcolor, UINT8 rightcolor) {
...
INT16 gametyperankings[NUMGAMETYPES] = {
    GT_COOP,  GT_COMPETITION, GT_RACE,

    GT_MATCH, GT_TEAMMATCH,

    GT_TAG,   GT_HIDEANDSEEK,
...
tolinfo_t TYPEOFLEVEL[NUMTOLNAMES] = {{"SOLO", TOL_SP},
                                      {"SP", TOL_SP},
                                      {"SINGLEPLAYER", TOL_SP},
                                      {"SINGLE", TOL_SP},
...
boolean G_GametypeUsesCoopLives(void) {
  return (gametyperules & (GTR_LIVES | GTR_FRIENDLY)) ==
         (GTR_LIVES | GTR_FRIENDLY);
}
...
//
boolean G_GametypeHasTeams(void) { return (gametyperules & GTR_TEAMS); }
...
boolean G_CoopGametype(void) {
  return ((gametyperules & (GTR_FRIENDLY | GTR_CAMPAIGN)) ==
          (GTR_FRIENDLY | GTR_CAMPAIGN));
}
...
//
boolean G_TagGametype(void) { return (gametyperules & GTR_TAG); }
...
static INT16 RandMap(UINT32 tolflags, INT16 pprevmap) {
  ...
  for (ix = 0; ix < NUMMAPS; ix++)
    if (mapheaderinfo[ix] &&
        (mapheaderinfo[ix]->typeoflevel & tolflags) == tolflags &&
...
// G_UpdateVisited
//
static void G_UpdateVisited(gamedata_t *data, player_t *player,
                            boolean global) {
  // Update visitation flags?
  if (!demoplayback && G_CoopGametype()            // Campaign mode
      ...
      && (global || player->pflags & PF_FINISHED)) // Actually beat the stage
  {
    ...
    // may seem incorrect but IS possible in what the main game uses as mp
    // special stages, and nummaprings will be -1 in NiGHTS
    ...
    if (!G_IsSpecialStage(gamemap)) {
      // not available to special stages because they can only really be done in
      // one order in an unmodified game, so impossible for first six and
      // trivial for seventh
    ...
    if ((earnedEmblems = M_CompletionEmblems(data)) && !global) {
      CONS_Printf(M_GetText("\x82"
                            "Earned %hu emblem%s for level completion.\n"),
                  (UINT16)earnedEmblems, earnedEmblems > 1 ? "s" : "");
    }
...
static boolean CanSaveLevel(INT32 mapnum) {
  ...
  return (mapheaderinfo[mapnum - 1] &&
          (mapheaderinfo[mapnum - 1]->levelflags & LF_SAVEGAME));
}
...
static void G_HandleSaveLevel(void) {
  ...
  // do this before running the intermission or custom cutscene, mostly for the
  // sake of marathon mode but it also massively reduces redundant file save
  // events in f_finale.c
  if (nextmap >= 1100 - 1) {
    if (!gamecomplete)
      gamecomplete = 2; // special temporary mode to prevent using SP level
                        // select in pause menu until the intermission is over
                        // without restricting it in every intermission
    if (cursaveslot > 0) {
      if (marathonmode) {
        ...
        cursaveslot = 0;
                 ...
                 !(netgame || multiplayer || ultimatemode || demorecording ||
                   metalrecording || modeattacking)) {
        G_SaveGame((UINT32)cursaveslot, spstage_start);
      }
    }
  }
  // and doing THIS here means you don't lose your progress if you close the
  // game mid-intermission
  else if (!(ultimatemode || netgame || multiplayer || demoplayback ||
             demorecording || metalrecording || modeattacking) &&
           !usedCheats && cursaveslot > 0 && CanSaveLevel(lastmap + 1)) {
    G_SaveGame((UINT32)cursaveslot,
               lastmap + 1); // not nextmap+1 to route around special stages
  }
}
...
INT16 G_GetNextMap(boolean ignoretokens, boolean silent) {
  ...
  if (!spec || nextmapoverride) {
    if (newmapnum >= 0 && newmapnum < NUMMAPS) {
      ...
      while (!mapheaderinfo[cm] ||
        ...
        {
          cm = newmapnum; // Start the loop again so that the error checking
                          // below is executed.
          ...
          if ((W_CheckNumForName(G_BuildMapName(cm + 1)) == LUMPERROR)) {
            if (!silent)
              CONS_Alert(CONS_ERROR,
                         M_GetText("Next map given (MAP %d) doesn't exist! "
                                   "Reverting to MAP01.\n"),
                         cm + 1);
            cm = 0;
            break;
        ...
        if (visitedmap[cm / 8] & (1 << (cm & 7))) // smells familiar
        {
          ...
          if (!silent)
            CONS_Alert(CONS_WARNING,
                       M_GetText("Can't find a compatible map after map %d; "
                                 "using map %d anyway\n"),
                       prevmap + 1, cm + 1);
          break;
        }
    ...
    // wrap around in race
    if (newmapnum >= 1100 - 1 && newmapnum <= 1102 - 1 &&
        !(gametyperules & GTR_CAMPAIGN))
      newmapnum = (INT16)(spstage_start - 1);

    if (newmapnum < 0 || (newmapnum >= NUMMAPS && newmapnum < 1100 - 1) ||
        newmapnum > 1103 - 1)
      I_Error("Followed map %d to invalid map %d\n", prevmap + 1,
              newmapnum + 1);

    if (!spec)
      lastmap = newmapnum; // Remember last map for when you come out of the
                           // special stage.
  }

  if (!ignoretokens &&
      (gottoken = ((gametyperules & GTR_SPECIALSTAGES) && token))) {
    token--;

    //		if (!nextmapoverride) // Having a token should pull the player
    // into the special stage before going to the overridden map (Issue #933)
    for (i = 0; i < 7; i++)
      if (!(emeralds & (1 << i))) {
        newmapnum = ((netgame || multiplayer) ? smpstage_start : sstage_start) +
                    i - 1; // to special stage!
        break;
      }
  ...
  if (spec && (!gottoken || ignoretokens) && !nextmapoverride)
    newmapnum = lastmap; // Exiting from a special stage? Go back to the game.
                         // Tails 08-11-2001
...
static void G_DoCompleted(void) {
  ...
  if ((skipstats && !modeattacking) || (modeattacking && stagefailed) ||
      (intertype == int_none)) {
    G_HandleSaveLevel();
    G_AfterIntermission();
  } else {
...
// See also F_EndCutscene, the only other place which handles intra-map/ending
// transitions
void G_AfterIntermission(void) {
  ...
  if (gamecomplete == 2) // special temporary mode to prevent using SP level
                         // select in pause menu until the intermission is over
                         // without restricting it in every intermission
    gamecomplete = 1;
  ...
  if ((gametyperules & GTR_CUTSCENES) &&
      mapheaderinfo[gamemap - 1]->cutscenenum && !modeattacking &&
      skipstats <= 1 && (gamecomplete || !(marathonmode & MA_NOCUTSCENES)) &&
      stagefailed == false) {
    // Start a custom cutscene.
    F_StartCustomCutscene(mapheaderinfo[gamemap - 1]->cutscenenum - 1, false,
                          false, false);
  } else {
    if (nextmap < 1100 - 1)
      G_NextLevel();
...
//
void G_NextLevel(void) { gameaction = ga_worlddone; }
...
static void G_DoWorldDone(void) {
  if (server) {
    ...
    if (gametyperules & GTR_CAMPAIGN)
      ...
      D_MapChange(nextmap + 1, gametype_to_use, ultimatemode, false, 0, false,
                  false);
    else
      // resetplayer in match/chaos/tag/CTF/race for more equality
      D_MapChange(nextmap + 1, gametype_to_use, ultimatemode, true, 0, false,
                  false);
...
// G_LoadGameData
// Loads the main data file, which stores information such as emblems found,
// etc.
void G_LoadGameData(gamedata_t *data) {
  ...
  savebuffer.size =
      FIL_ReadFile(va(pandf, srb2home, gamedatafilename), &savebuffer.buf);
  if (!savebuffer.size) {
...
#ifdef COMPAT_GAMEDATA_ID // Account for lower MAXUNLOCKABLES and
                          // MAXEXTRAEMBLEMS from older versions
  ...
  if (versionID == COMPAT_GAMEDATA_ID) {
    ...
    FIL_WriteFile(va(pandf, srb2home, backupfilename), &savebuffer.buf,
                  savebuffer.size);
  } else
#endif
  {
    ...
    UINT32 expectedID =
        quickncasehash(timeattackfolder, sizeof timeattackfolder);
  ...
  // which can accidentally delete players' legitimate data if the code ever has
  // any tiny mistakes!
  data->loaded = true;
...
// G_SaveGameData
// Saves the main data file, which stores information such as emblems found,
// etc.
void G_SaveGameData(gamedata_t *data) {
  ...
  if (!savebuffer.buf) {
    CONS_Alert(CONS_ERROR,
               M_GetText("No more free memory for saving game data\n"));
    return;
  }
  savebuffer.pos = 0;
  ...
  for (i = 0; i < NUMMAPS; i++) {
    ...
    for (curmare = 0; curmare < (data->nightsrecords[i]->nummares + 1);
         ++curmare) {
  ...
  FIL_WriteFile(va(pandf, srb2home, gamedatafilename), savebuffer.buf,
                savebuffer.pos);
  free(savebuffer.buf);
}
...
void G_LoadGame(UINT32 slot, INT16 mapoverride) {
  ...
  // memset savedata to all 0, fixes calling perfectly valid saves corrupt
  // because of bots
  ...
  if (strcmp((const char *)&savebuffer.buf[savebuffer.pos],
             (const char *)vcheck)) {
#ifdef SAVEGAME_OTHERVERSIONS
    M_StartMessage(
        M_GetText("Save game from different version.\nYou can load this "
...
void G_SaveGame(UINT32 slot, INT16 mapnum) {
  ...
  gameaction = ga_nothing;
  {
    ...
    if (marathonmode) {
      ...
      if (!(marathonmode & MA_INGAME))
        writetime +=
            TICRATE * 5; // live event backup penalty because we don't know how
                         // long it takes to get to the next map
  ...
  if (cv_debug && saved)
    ...
    CONS_Alert(
        CONS_ERROR,
        M_GetText("Error while writing to %s for save slot %u, base: %s\n"),
...
void G_SaveGameOver(UINT32 slot, boolean modifylives) {
...

  {
    ...
    if (strcmp(temp, timeattackfolder))
      BADSAVE
...
//
void G_DeferedInitNew(boolean pultmode, const char *mapname, INT32 character,
  ...
  if (mapname)
    D_MapChange(M_MapNumber(mapname[3], mapname[4]), gametype, pultmode, true,
                1, false, FLS);
}
...
void G_InitNew(UINT8 pultmode, const char *mapname, boolean resetplayer,
  ...
  if (!demoplayback && !netgame) // Netgame sets random seed elsewhere, demo
                                 // playback sets seed just before us!
    P_SetRandSeed(M_RandomizedSeed()); // Use a more "Random" random seed

  if (resetplayer) {
    // Clear a bunch of variables
    numgameovers = tokenlist = token = sstimer = redscore = bluescore =
        lastmap = 0;
    countdown = countdown2 = exitfadestarted = 0;
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      players[i].playerstate = PST_REBORN;
      players[i].starpostscale = players[i].starpostangle =
          players[i].starpostnum = players[i].starposttime = 0;
    ...
    if (!dedicated) // except in dedicated servers, where it is not registered
                    // and can actually I_Error debug builds
      CV_StealthSetValue(&cv_itemfinder, 0);
  }

  // Restore each player's skin if it was previously forced to be a specific one
  // (Looks a bit silly, but it works.)
  boolean reset_skin = netgame && mapheaderinfo[gamemap - 1] &&
                       mapheaderinfo[gamemap - 1]->forcecharacter[0] != '\0';

  // internal game map
  // well this check is useless because it is done before
  // (d_netcmd.c::command_map_f) but in case of for demos....
  ...
  if ((gametyperules & GTR_CUTSCENES) && !skipprecutscene &&
      mapheaderinfo[gamemap - 1]->precutscenenum && !modeattacking &&
      !(marathonmode & MA_NOCUTSCENES)) // Start a custom cutscene.
    F_StartCustomCutscene(mapheaderinfo[gamemap - 1]->precutscenenum - 1, true,
                          resetplayer, FLS);
  else
...
char *G_BuildMapTitle(INT32 mapnum) {
  ...
  if (strcmp(mapheaderinfo[mapnum - 1]->lvlttl, "")) {
    ...
    if (actnum > 0)
      sprintf(title + strlen(title), " %d", actnum);
  }

  return title;
}

static void measurekeywords(mapsearchfreq_t *fr, struct searchdim **dimp,
                            UINT8 *cuntp, const char *s, const char *q,
                            boolean wanttable) {
  char *qp;
  char *sp;
  if (wanttable)
    (*dimp) =
        Z_Realloc((*dimp), 255 * sizeof(struct searchdim), PU_STATIC, NULL);
  for (qp = strtok(va("%s", q), " "); qp && fr->total < 255;
       qp = strtok(0, " ")) {
  ...
  if (wanttable)
    (*dimp) = Z_Realloc((*dimp), (*cuntp) * sizeof(struct searchdim), PU_STATIC,
                        NULL);
}

static void writesimplefreq(mapsearchfreq_t *fr, INT32 *frc, INT32 mapnum,
                            UINT8 pos, UINT8 siz) {
    ...
    if (mapheaderinfo[i]) {
      ...
      if (strnicmp(realmapname, mapname, mapnamelen) == 0) {
        ...
        if ((aprop = strcasestr(realmapname, mapname))) {
          if (wanttable) {
            writesimplefreq(freq, &freqc, mapnum, aprop - realmapname,
                            mapnamelen);
          }
          if (apromapnum == 0) {
            apromapnum = mapnum;
        ...
        } else /* ...match individual keywords */
        {
          ...
          measurekeywords(&freq[freqc], &freq[freqc].matchd,
                          &freq[freqc].matchc, realmapname, mapname, wanttable);
          measurekeywords(&freq[freqc], &freq[freqc].keywhd,
                          &freq[freqc].keywhc, mapheaderinfo[i]->keywords,
                          mapname, wanttable);
          if (freq[freqc].total)
            freqc++;
        }
  ...
  if (mapnamelen == 1) {
    ...
    } else if (mapname[0] == '+' && mapheaderinfo[gamemap - 1]) // next map
    {
      ...
      if (newmapnum < 1 || newmapnum > NUMMAPS) {
        CONS_Alert(CONS_ERROR,
                   M_GetText("NextLevel (%d) is not a valid map.\n"),
                   newmapnum);
        return 0;
      }
    }
...
//
void G_SetGamestate(gamestate_t newstate) { gamestate = newstate; }
...
void G_SetExitGameFlag(void) { exitgame = true; }

void G_ClearExitGameFlag(void) { exitgame = false; }

boolean G_GetExitGameFlag(void) { return exitgame; }

// Same deal with retrying.
void G_SetRetryFlag(void) { retrying = true; }

void G_ClearRetryFlag(void) { retrying = false; }

boolean G_GetRetryFlag(void) { return retrying; }
...
void G_ClearModeAttackRetryFlag(void) { retryingmodeattack = false; }

boolean G_GetModeAttackRetryFlag(void) { return retryingmodeattack; }

// Time utility functions
INT32 G_TicsToHours(tic_t tics) { return tics / (3600 * TICRATE); }
...
INT32 G_TicsToSeconds(tic_t tics) { return (tics / TICRATE) % 60; }
