...
hudinfo_t hudinfo[NUMHUDITEMS] = {
    ...
    {16, 82, V_SNAPTOLEFT | V_SNAPTOTOP},  // HUD_RINGS
    {96, 82, V_SNAPTOLEFT | V_SNAPTOTOP},  // HUD_RINGSNUM
    {120, 82, V_SNAPTOLEFT | V_SNAPTOTOP}, // HUD_RINGSNUMTICS

    {16, 50, V_SNAPTOLEFT | V_SNAPTOTOP},  // HUD_SCORE
    {120, 50, V_SNAPTOLEFT | V_SNAPTOTOP}, // HUD_SCORENUM

    {16, 66, V_SNAPTOLEFT | V_SNAPTOTOP},  // HUD_TIME
    {72, 66, V_SNAPTOLEFT | V_SNAPTOTOP},  // HUD_MINUTES
    {72, 66, V_SNAPTOLEFT | V_SNAPTOTOP},  // HUD_TIMECOLON
    {96, 66, V_SNAPTOLEFT | V_SNAPTOTOP},  // HUD_SECONDS
    {96, 66, V_SNAPTOLEFT | V_SNAPTOTOP},  // HUD_TIMETICCOLON
    {120, 66, V_SNAPTOLEFT | V_SNAPTOTOP}, // HUD_TICS
...
// NiGHTS link colors; 3 sets with increasingly fancy colors (1 to 299, 300 to
// 599, 600 and above)
skincolornum_t linkColor[3][NUMLINKCOLORS] = {
    {SKINCOLOR_SHAMROCK, SKINCOLOR_AQUA, SKINCOLOR_SKY, SKINCOLOR_BLUE,
     SKINCOLOR_PURPLE, SKINCOLOR_MAGENTA, SKINCOLOR_ROSY, SKINCOLOR_RED,
     SKINCOLOR_ORANGE, SKINCOLOR_GOLD, SKINCOLOR_YELLOW, SKINCOLOR_PERIDOT},
    {SKINCOLOR_EMERALD, SKINCOLOR_OCEAN, SKINCOLOR_AQUAMARINE,
     SKINCOLOR_SAPPHIRE, SKINCOLOR_GALAXY, SKINCOLOR_SIBERITE, SKINCOLOR_TAFFY,
     SKINCOLOR_RUBY, SKINCOLOR_GARNET, SKINCOLOR_TOPAZ, SKINCOLOR_LEMON,
     SKINCOLOR_LIME},
    {SKINCOLOR_ISLAND, SKINCOLOR_TURQUOISE, SKINCOLOR_DREAM, SKINCOLOR_DAYBREAK,
     SKINCOLOR_VAPOR, SKINCOLOR_FUCHSIA, SKINCOLOR_VIOLET, SKINCOLOR_EVENTIDE,
     SKINCOLOR_KETCHUP, SKINCOLOR_FOUNDATION, SKINCOLOR_HEADLIGHT,
...
void ST_doPaletteStuff(void) {
  ...
  if (rendermode == render_opengl && !HWR_ShouldUsePaletteRendering())
    palette =
        0; // Don't set the palette to a flashpal in OpenGL's truecolor mode
#endif
...
void ST_UnloadGraphics(void) { Patch_FreeTag(PU_HUDGFX); }
...
void ST_LoadGraphics(void) {
  ...
  sboperiod =
      W_CachePatchName("STTPERIO", PU_HUDGFX); // Period for time centiseconds
...
void ST_LoadFaceGraphics(INT32 skinnum) {
  if (skins[skinnum]->sprites[SPR2_XTRA].numframes > XTRA_LIFEPIC) {
    ...
    spritedef_t *super_sprdef =
        P_GetSkinSpritedef(skins[skinnum], SPR2_XTRA | SPR2F_SUPER);
    if (super_sprdef->numframes > XTRA_LIFEPIC) {
      sprdef = super_sprdef;
      sprframe = &sprdef->spriteframes[0];
      superprefix[skinnum] = W_CachePatchNum(sprframe->lumppat[0], PU_HUDGFX);
    } else
      superprefix[skinnum] = faceprefix[skinnum]; // not manually freed, okay to
                                                  // set to same pointer
  } else
    faceprefix[skinnum] = superprefix[skinnum] =
        W_CachePatchName("MISSING", PU_HUDGFX); // ditto
}
...
//
// Initializes the status bar, sets the defaults border patch for the window
// borders.
//

// used by OpenGL mode, holds lumpnum of flat used to fill space around the
// viewwindow
...
//                          INTERNAL DRAWING
// =========================================================================
#define ST_DrawTopLeftOverlayPatch(x, y, p)                                    \
  V_DrawScaledPatch(x, y,                                                      \
                    V_PERPLAYER | V_SNAPTOTOP | V_SNAPTOLEFT | V_HUDTRANS, p)
#define ST_DrawNumFromHud(h, n, flags)                                         \
  V_DrawTallNum(hudinfo[h].x, hudinfo[h].y,                                    \
                hudinfo[h].f | V_PERPLAYER | flags, n)
#define ST_DrawPadNumFromHud(h, n, q, flags)                                   \
  V_DrawPaddedTallNum(hudinfo[h].x, hudinfo[h].y,                              \
                      hudinfo[h].f | V_PERPLAYER | flags, n, q)
#define ST_DrawPatchFromHud(h, p, flags)                                       \
  V_DrawScaledPatch(hudinfo[h].x, hudinfo[h].y,                                \
                    hudinfo[h].f | V_PERPLAYER | flags, p)
...
static void ST_DrawNightsOverlayNum(fixed_t x /* right border */, fixed_t y,
                                    fixed_t s, INT32 a, UINT32 num,
                                    patch_t **numpat, skincolornum_t colornum) {
...
#define V_DrawDebugLine(str)                                                   \
  if (lowh && (height > lowh)) {                                               \
    V_DrawRightAlignedThinString(320, 8 + lowh, VFLAGS | V_REDMAP,             \
                                 "SOME INFO NOT VISIBLE");                     \
    return;                                                                    \
  }                                                                            \
  V_DrawAlignedFontString(320, height, VFLAGS, textscale, textscale, str,      \
                          hu_font, alignright);                                \
  height += h;

#define V_DrawDebugFlag(f, str)                                                \
  V_DrawAlignedFontString(width, height, VFLAGS | f, textscale, textscale,     \
                          str, hu_font, alignright);                           \
  width -= w
  ...
  if (cv_debug & DBG_PLAYER) {
    ...
    V_DrawDebugLine(va("AIR: %4d, %3d", stplyr->powers[pw_underwater],
                       stplyr->powers[pw_spacetime]));
    V_DrawDebugLine(
        va("ABILITY: %3d, %3d", stplyr->charability, stplyr->charability2));
    ...
    // Flags
    V_DrawDebugFlag(
        ((stplyr->pflags & PF_SHIELDABILITY) ? V_GREENMAP : V_REDMAP), "SH");
    V_DrawDebugFlag(((stplyr->pflags & PF_THOKKED) ? V_GREENMAP : V_REDMAP),
                    "TH");
    V_DrawDebugFlag(((stplyr->pflags & PF_STARTDASH) ? V_GREENMAP : V_REDMAP),
                    "ST");
    V_DrawDebugFlag(((stplyr->pflags & PF_SPINNING) ? V_GREENMAP : V_REDMAP),
                    "SP");
    V_DrawDebugFlag(
        ((stplyr->pflags & PF_NOJUMPDAMAGE) ? V_GREENMAP : V_REDMAP), "ND");
    V_DrawDebugFlag(((stplyr->pflags & PF_JUMPED) ? V_GREENMAP : V_REDMAP),
                    "JD");
    V_DrawDebugFlag(((stplyr->pflags & PF_STARTJUMP) ? V_GREENMAP : V_REDMAP),
                    "SJ");
    V_DrawDebugFlag(0, "PF/SF:");
    height += h;
    width = 320;
    V_DrawDebugFlag(((stplyr->pflags & PF_INVIS) ? V_GREENMAP : V_REDMAP),
                    "*I");
    V_DrawDebugFlag(((stplyr->pflags & PF_NOCLIP) ? V_GREENMAP : V_REDMAP),
                    "*C");
    V_DrawDebugFlag(((stplyr->pflags & PF_GODMODE) ? V_GREENMAP : V_REDMAP),
                    "*G");
    V_DrawDebugFlag(((stplyr->charflags & SF_SUPER) ? V_GREENMAP : V_REDMAP),
                    "SU");
    V_DrawDebugFlag(
        ((stplyr->pflags & PF_APPLYAUTOBRAKE) ? V_GREENMAP : V_REDMAP), "AA");
    V_DrawDebugFlag(((stplyr->pflags & PF_SLIDING) ? V_GREENMAP : V_REDMAP),
                    "SL");
    V_DrawDebugFlag(((stplyr->pflags & PF_BOUNCING) ? V_GREENMAP : V_REDMAP),
                    "BO");
    V_DrawDebugFlag(((stplyr->pflags & PF_GLIDING) ? V_GREENMAP : V_REDMAP),
                    "GL");
    height += h;
  ...
  if (cv_debug & DBG_DETAILED) {
    ...
    width = 320;
    V_DrawDebugFlag(
        ((stplyr->mo->eflags & MFE_APPLYPMOMZ) ? V_GREENMAP : V_REDMAP), "AP");
    V_DrawDebugFlag(((stplyr->mo->eflags & MFE_SPRUNG) ? V_GREENMAP : V_REDMAP),
                    "SP");
    // V_DrawDebugFlag(((stplyr->mo->eflags & MFE_PUSHED)          ? V_GREENMAP
    // : V_REDMAP), "PU"); -- not relevant to players
    V_DrawDebugFlag(
        ((stplyr->mo->eflags & MFE_GOOWATER) ? V_GREENMAP : V_REDMAP), "GW");
    V_DrawDebugFlag(
        ((stplyr->mo->eflags & MFE_VERTICALFLIP) ? V_GREENMAP : V_REDMAP),
        "VF");
    V_DrawDebugFlag(
        ((stplyr->mo->eflags & MFE_JUSTSTEPPEDDOWN) ? V_GREENMAP : V_REDMAP),
        "JS");
    V_DrawDebugFlag(
        ((stplyr->mo->eflags & MFE_UNDERWATER) ? V_GREENMAP : V_REDMAP), "UW");
    V_DrawDebugFlag(
        ((stplyr->mo->eflags & MFE_TOUCHWATER) ? V_GREENMAP : V_REDMAP), "TW");
    V_DrawDebugFlag(
        ((stplyr->mo->eflags & MFE_JUSTHITFLOOR) ? V_GREENMAP : V_REDMAP),
        "JH");
    V_DrawDebugFlag(
        ((stplyr->mo->eflags & MFE_ONGROUND) ? V_GREENMAP : V_REDMAP), "OG");
...
static void ST_drawScore(void) {
  ...
  if (objectplacing) {
    if (op_displayflags > UINT16_MAX)
      ST_DrawTopLeftOverlayPatch((hudinfo[HUD_SCORENUM].x - tallminus->width),
                                 hudinfo[HUD_SCORENUM].y, tallminus);
    else
...
static void ST_drawRaceNum(INT32 time) {
  ...
  V_DrawScaledPatch(((BASEVIDWIDTH - racenum->width) / 2), height, V_PERPLAYER,
                    racenum);
}
...
static void ST_drawTime(void) {
  ...
  if (objectplacing) {
    ...
    if ((gametyperules & GTR_STARTCOUNTDOWN) &&
        (stplyr->realtime <= (hidetime * TICRATE))) {
      ...
      // Hidetime finish!
      if ((gametyperules & GTR_STARTCOUNTDOWN) &&
          (stplyr->realtime < ((hidetime + 1) * TICRATE)))
        ST_drawRaceNum(hidetime * TICRATE - stplyr->realtime);

      // Time limit?
      if ((gametyperules & GTR_TIMELIMIT) && cv_timelimit.value &&
          timelimitintics > 0) {
  ...
  downwards = (downwards && (tics < 30 * TICRATE) && (leveltime / 5 & 1) &&
               !stoppedclock); // overtime?
  ...
  if (cv_timetic.value == 3) // Tics only -- how simple is this?
    ...
    if (cv_timetic.value == 1 || cv_timetic.value == 2 || modeattacking ||
        marathonmode) {
...
static inline void ST_drawRings(void) {
  ...
  ST_DrawPatchFromHud(
      HUD_RINGS,
      ((!stplyr->spectator && stplyr->rings <= 0 && leveltime / 5 & 1)
           ? sboredrings
           : sborings),
      ((stplyr->spectator) ? V_HUDTRANSHALF : V_HUDTRANS));

  if (objectplacing)
    ringnum = op_currentdoomednum;
  else if (stplyr->rings < 0 || stplyr->spectator ||
           stplyr->playerstate == PST_REBORN)
    ringnum = 0;
  else
    ringnum = stplyr->rings;

  if (cv_timetic.value == 2) // Yes, even in modeattacking
    ST_DrawNumFromHud(HUD_RINGSNUMTICS, ringnum,
                      V_PERPLAYER |
                          ((stplyr->spectator) ? V_HUDTRANSHALF : V_HUDTRANS));
  else
    ST_DrawNumFromHud(HUD_RINGSNUM, ringnum,
                      V_PERPLAYER |
                          ((stplyr->spectator) ? V_HUDTRANSHALF : V_HUDTRANS));
}
...
static void ST_drawLivesArea(void) {
  ...
  if (stplyr->spectator) {
    // spectator face
    UINT8 *colormap =
        R_GetTranslationColormap(stplyr->skin, SKINCOLOR_CLOUDY, GTC_CACHE);
    V_DrawSmallMappedPatch(hudinfo[HUD_LIVES].x, hudinfo[HUD_LIVES].y,
                           hudinfo[HUD_LIVES].f | V_PERPLAYER | V_HUDTRANSHALF,
                           faceprefix[stplyr->skin], colormap);
  } else if (stplyr->mo && stplyr->mo->color) {
    // skincolor face/super
    UINT8 *colormap =
        R_GetTranslationColormap(stplyr->skin, stplyr->mo->color, GTC_CACHE);
    ...
    V_DrawSmallMappedPatch(hudinfo[HUD_LIVES].x, hudinfo[HUD_LIVES].y,
                           hudinfo[HUD_LIVES].f | V_PERPLAYER | V_HUDTRANS,
                           face, colormap);
    if (st_translucency == 10 && stplyr->powers[pw_super] == 1 &&
        stplyr->mo->tracer) {
      INT32 v_supertrans =
          (stplyr->mo->tracer->frame & FF_TRANSMASK) >> FF_TRANSSHIFT;
      if (v_supertrans < 10) {
        v_supertrans <<= V_ALPHASHIFT;
        colormap = R_GetTranslationColormap(
            stplyr->skin, stplyr->mo->tracer->color, GTC_CACHE);
        V_DrawSmallMappedPatch(
            hudinfo[HUD_LIVES].x, hudinfo[HUD_LIVES].y,
    ...
    // skincolor face
    UINT8 *colormap =
        R_GetTranslationColormap(stplyr->skin, facecolor, GTC_CACHE);
    V_DrawSmallMappedPatch(hudinfo[HUD_LIVES].x, hudinfo[HUD_LIVES].y,
                           hudinfo[HUD_LIVES].f | V_PERPLAYER | V_HUDTRANS,
                           faceprefix[stplyr->skin], colormap);
  }
  ...
  // Spectator
    ...
    if (G_GametypeUsesLives()) {
      ...
      if ((netgame || multiplayer) && G_GametypeUsesCoopLives() &&
        ...
        livescount = (((netgame || multiplayer) && G_GametypeUsesCoopLives() &&
    ...
    if (candrawlives) {
      ...
      if (livescount == INFLIVES) {
        V_DrawCharacter(hudinfo[HUD_LIVES].x + 50, hudinfo[HUD_LIVES].y + 8,
                        '\x16' | 0x80 | hudinfo[HUD_LIVES].f | V_PERPLAYER |
                            V_HUDTRANS,
                        false);
      } else {
        if (stplyr->playerstate == PST_DEAD && !(stplyr->spectator) &&
            (livescount || stplyr->deadtimer < (TICRATE << 1)) &&
            !(stplyr->pflags & PF_FINISHED))
          livescount++;
        if (livescount > 99)
          livescount = 99;
        V_DrawRightAlignedString(
            hudinfo[HUD_LIVES].x + 58, hudinfo[HUD_LIVES].y + 8,
            hudinfo[HUD_LIVES].f | V_PERPLAYER |
      ...
      if (G_TagGametype() && (stplyr->pflags & PF_TAGIT)) {
        V_DrawRightAlignedString(
            hudinfo[HUD_LIVES].x + 58, hudinfo[HUD_LIVES].y + 8,
            V_HUDTRANS | hudinfo[HUD_LIVES].f | V_PERPLAYER, "IT!");
      } else if (G_GametypeHasTeams()) {
        if (stplyr->ctfteam == 1) {
          V_DrawRightAlignedString(
              hudinfo[HUD_LIVES].x + 58, hudinfo[HUD_LIVES].y + 8,
              V_HUDTRANS | hudinfo[HUD_LIVES].f | V_PERPLAYER, "RED");
        } else if (stplyr->ctfteam == 2) {
          V_DrawRightAlignedString(
              hudinfo[HUD_LIVES].x + 58, hudinfo[HUD_LIVES].y + 8,
              V_HUDTRANS | hudinfo[HUD_LIVES].f | V_PERPLAYER, "BLUE");
        }
      }
    }
#undef ST_drawLivesX
  ...
  if (strlen(skins[stplyr->skin]->hudname) <= 5)
    V_DrawRightAlignedString(hudinfo[HUD_LIVES].x + 58, hudinfo[HUD_LIVES].y,
                             v_colmap, skins[stplyr->skin]->hudname);
  else if (V_StringWidth(skins[stplyr->skin]->hudname, v_colmap) <= 48)
    V_DrawString(hudinfo[HUD_LIVES].x + 18, hudinfo[HUD_LIVES].y, v_colmap,
                 skins[stplyr->skin]->hudname);
  else if (V_ThinStringWidth(skins[stplyr->skin]->hudname, v_colmap) <= 40)
    V_DrawRightAlignedThinString(hudinfo[HUD_LIVES].x + 58,
                                 hudinfo[HUD_LIVES].y, v_colmap,
                                 skins[stplyr->skin]->hudname);
  else
    V_DrawThinString(hudinfo[HUD_LIVES].x + 18, hudinfo[HUD_LIVES].y, v_colmap,
                     skins[stplyr->skin]->hudname);

  // Power Stones collected
  if (G_RingSlingerGametype() && LUA_HudEnabled(hud_powerstones)) {
    INT32 workx = hudinfo[HUD_LIVES].x + 1, j;
    if ((leveltime & 1) && stplyr->powers[pw_invulnerability] &&
        (stplyr->powers[pw_sneakers] ==
         stplyr->powers[pw_invulnerability])) // hack; extremely unlikely to be
                                              // activated unintentionally
    {
      for (j = 0; j < 7; ++j) // "super" indicator
      {
        V_DrawScaledPatch(workx, hudinfo[HUD_LIVES].y - 9,
...
static void ST_drawInput(void) {
  ...
  const INT32 accent =
      V_SNAPTOLEFT | V_SNAPTOBOTTOM | (color ? skincolors[color].ramp[4] : 0);
  INT32 col;
  UINT8 offs;
  ...
  if (cv_showinputjoy.value) // joystick render!
  {
    ...
    if (stplyr->cmd.sidemove || stplyr->cmd.forwardmove) {
      ...
      V_DrawFill(x + 3 + stplyr->cmd.sidemove / 12,
                 y + 2 - stplyr->cmd.forwardmove / 12, 10, 10,
                 hudinfo[HUD_INPUT].f | 29);
      V_DrawFill(x + 3 + stplyr->cmd.sidemove / 9,
                 y + 1 - stplyr->cmd.forwardmove / 9, 10, 10, accent);
    } else {
      // just a limited, greyed out joystick top
      V_DrawFill(x + 3, y + 11, 10, 1, hudinfo[HUD_INPUT].f | 29);
      V_DrawFill(x + 3, y + 1, 10, 10, hudinfo[HUD_INPUT].f | 16);
    }
  } else // arrows!
  {
    // <
...
#define drawbutt(xoffs, yoffs, butt, symb)                                     \
  if (stplyr->cmd.buttons & butt) {                                            \
    ...
    offs = 1;                                                                  \
    col = hudinfo[HUD_INPUT].f | 16;                                           \
    V_DrawFill(x + 16 + (xoffs), y + 9 + (yoffs), 10, 1,                       \
               hudinfo[HUD_INPUT].f | 29);                                     \
  }                                                                            \
  V_DrawFill(x + 16 + (xoffs), y + (yoffs) - offs, 10, 10, col);               \
  V_DrawCharacter(x + 16 + 1 + (xoffs), y + 1 + (yoffs) - offs,                \
                  hudinfo[HUD_INPUT].f | symb, false)

  drawbutt(4, -3, BT_JUMP, 'J');
  drawbutt(15, -3, BT_SPIN, 'S');

  V_DrawFill(x + 16 + 4, y + 8, 21, 10,
             hudinfo[HUD_INPUT].f | 20); // sundial backing
  if (stplyr->mo) {
    UINT8 i, precision;
    angle_t ang = (stplyr->powers[pw_carry] == CR_NIGHTSMODE)
                      ? (FixedAngle((stplyr->flyangle - 90) << FRACBITS) >>
                         ANGLETOFINESHIFT)
                      : (stplyr->mo->angle -
                         R_PointToAngle(stplyr->mo->x, stplyr->mo->y)) >>
                            ANGLETOFINESHIFT;
    ...
    if (ycomp > 0)
      V_DrawFill(x + 16 + 13 - xcomp, y + 11 - ycomp, 3, 3,
                 accent); // point (behind)
    ...
    for (i = 0; i < precision; i++) // line
    {
      V_DrawFill(x + 16 + 14 - (i * xcomp) / precision,
                 y + 12 - (i * ycomp) / precision, 1, 1,
                 hudinfo[HUD_INPUT].f | 16);
    }

    if (ycomp <= 0)
      V_DrawFill(x + 16 + 13 - xcomp, y + 11 - ycomp, 3, 3,
                 accent); // point (in front)
  }
...
#undef drawbutt
      ...
      V_DrawThinString(
          ...
          hudinfo[HUD_INPUT].f |
              ((!stplyr->powers[pw_carry] &&
                ...
                (!stplyr->capsule ||
                 (stplyr->capsule->reactiontime != (stplyr - players) + 1)))
                   ? 0
                   : V_GRAYMAP),
          "AUTOBRAKE");
      y -= 8;
  ...
  if (!demosynced) // should always be last, so it doesn't push anything else
                   // around
    V_DrawThinString(
        x, y, hudinfo[HUD_INPUT].f | ((leveltime & 4) ? V_YELLOWMAP : V_REDMAP),
        "BAD DEMO!!");
}
...
//
void ST_stopTitleCard(void) { lt_active = false; }
...
void ST_drawTitleCard(void) {
  ...
  if (!splitscreen || (splitscreen && stplyr == &players[displayplayer])) {
    zzticker = lt_ticker;
    V_DrawMappedPatch(FixedInt(lt_zigzag), (-zzticker) % zigzag->height,
                      V_SNAPTOTOP | V_SNAPTOLEFT, zigzag, colormap);
    V_DrawMappedPatch(FixedInt(lt_zigzag),
                      (zigzag->height - zzticker) % zigzag->height,
                      V_SNAPTOTOP | V_SNAPTOLEFT, zigzag, colormap);
    V_DrawMappedPatch(FixedInt(lt_zigzag),
                      (-zztext->height + zzticker) % zztext->height,
                      V_SNAPTOTOP | V_SNAPTOLEFT, zztext, colormap);
    V_DrawMappedPatch(FixedInt(lt_zigzag), (zzticker) % zztext->height,
                      V_SNAPTOTOP | V_SNAPTOLEFT, zztext, colormap);
  }
  ...
  if (actnum) {
    if (!splitscreen) {
      if (actnum >
          9) // slightly offset the act diamond for two-digit act numbers
        V_DrawMappedPatch(ttlnumxpos + (V_LevelActNumWidth(actnum) / 4) +
                              ttlscroll,
                          104 - ttlscroll, 0, actpat, colormap);
      else
        V_DrawMappedPatch(ttlnumxpos + ttlscroll, 104 - ttlscroll, 0, actpat,
                          colormap);
    }
  ...
  V_DrawCenteredString(subttlxpos - ttlscroll, 135,
                       V_PERPLAYER | V_ALLOWLOWERCASE, subttl);
...
static void ST_drawPowerupHUD(void) {
  ...
  const UINT8 q =
      ((splitscreen && stplyr == &players[secondarydisplayplayer]) ? 1 : 0);
  static INT32 flagoffs[2] = {0, 0}, shieldoffs[2] = {0, 0},
               finishoffs[2] = {0, 0}, stackoffs[2] = {0, 0};
  ...
  if (stplyr->pflags & PF_FINISHED && cv_exitmove.value && multiplayer) {
    ...
    V_DrawSmallScaledPatch(offs, hudinfo[HUD_POWERUPS].y,
                           V_PERPLAYER | hudinfo[HUD_POWERUPS].f | V_HUDTRANS,
                           fnshico);
  ...
  if (stplyr->powers[pw_shield] & SH_NOSTACK) {
    ...
    if ((stplyr->powers[pw_shield] & SH_NOSTACK & ~SH_FORCEHP) == SH_FORCE &&
        (stplyr->powers[pw_shield] & SH_FORCEHP) >
            0) // Special handling for >1HP Force Shields
    {
      UINT8 max = (stplyr->powers[pw_shield] & SH_FORCEHP);

      V_DrawSmallScaledPatch(
          offs, hudinfo[HUD_POWERUPS].y,
          V_PERPLAYER | hudinfo[HUD_POWERUPS].f | V_HUDTRANSHALF, forceshield);
      V_DrawSmallScaledPatch(offs - 2, hudinfo[HUD_POWERUPS].y - 2,
                             V_PERPLAYER | hudinfo[HUD_POWERUPS].f | V_HUDTRANS,
                             forceshield);

      if (max > 1) // if the shield has more than 2 hits, show the extra n hits
                   // as "+n"
        V_DrawRightAlignedThinString(offs + 16, hudinfo[HUD_POWERUPS].y,
                                     V_PERPLAYER | hudinfo[HUD_POWERUPS].f |
                                         V_HUDTRANS,
                                     va("+%d", max - 1));
    } else {
      ...
      if (p)
        V_DrawSmallScaledPatch(
            offs, hudinfo[HUD_POWERUPS].y,
            V_PERPLAYER | hudinfo[HUD_POWERUPS].f | V_HUDTRANS, p);
    }
...
#define DRAWTIMERICON(patch, timer)                                            \
  V_DrawSmallScaledPatch(offs, hudinfo[HUD_POWERUPS].y,                        \
                         V_PERPLAYER | hudinfo[HUD_POWERUPS].f | V_HUDTRANS,   \
                         patch);                                               \
  V_DrawRightAlignedThinString(offs + 16, hudinfo[HUD_POWERUPS].y + 8,         \
                               V_PERPLAYER | hudinfo[HUD_POWERUPS].f,          \
                               va("%d", timer / TICRATE));

  // Invincibility, both from monitor and after being hit
  invulntime = stplyr->powers[pw_flashing] ? stplyr->powers[pw_flashing]
                                           : stplyr->powers[pw_invulnerability];
  // Note: pw_flashing always makes the icon flicker regardless of time, unlike
  // pw_invulnerability
  if (stplyr->powers[pw_invulnerability] > 3 * TICRATE ||
      (invulntime && leveltime & 1)) {
  ...
  // Super Sneakers
  if (stplyr->powers[pw_sneakers] > 3 * TICRATE ||
      (stplyr->powers[pw_sneakers] && leveltime & 1)) {
  ...
  // Gravity Boots
  if (stplyr->powers[pw_gravityboots] > 3 * TICRATE ||
      (stplyr->powers[pw_gravityboots] && leveltime & 1)) {
  ...
  // Nightopian Helper
  if (stplyr->powers[pw_nights_helper] > 3 * TICRATE ||
      (stplyr->powers[pw_nights_helper] && leveltime & 1)) {
  ...
  // Link Freeze
  if (stplyr->powers[pw_nights_linkfreeze] > 3 * TICRATE ||
      (stplyr->powers[pw_nights_linkfreeze] && leveltime & 1)) {
  ...
  // Super Paraloop
  if (stplyr->powers[pw_nights_superloop] > 3 * TICRATE ||
      (stplyr->powers[pw_nights_superloop] && leveltime & 1)) {
...
static void ST_drawFirstPersonHUD(void) {
  ...
  if (stplyr->powers[pw_underwater] && stplyr->powers[pw_spacetime])
    ...
    airtime = (stplyr->powers[pw_spacetime]) ? stplyr->powers[pw_spacetime]
                                             : stplyr->powers[pw_underwater];

  if (airtime < 1)
    return; // No air timers are active, nothing would be drawn anyway

  airtime--; // The original code was all n*TICRATE + 1, so let's remove 1 tic
             // for simplicity
  ...
  airtime /=
      (2 *
       TICRATE); // To be strictly accurate it'd need to be ((airtime/TICRATE) -
                 // 1)/2, but integer division rounds down for us
  ...
  if (p && !F_GetPromptHideHud(60 - p->topoffset))
    V_DrawScaledPatch(
        (BASEVIDWIDTH / 2) - (p->width / 2) + SHORT(p->leftoffset),
        60 - SHORT(p->topoffset), V_PERPLAYER | V_PERPLAYER | V_TRANSLUCENT, p);
}
...
static void ST_drawNightsRecords(void) {
  ...
  {
    V_DrawCenteredString(BASEVIDWIDTH / 2, 52, V_GREENMAP | aflag,
                         M_GetText("GET TO THE GOAL!"));
    V_DrawCenteredString(BASEVIDWIDTH / 2, 60, aflag,
                         M_GetText("SCORE MULTIPLIER START!"));
    ...
    if (stplyr->finishedtime) {
      ...
      V_DrawRightAlignedString(
          BASEVIDWIDTH / 2 + 48, 140, aflag,
          va("%i:%02i.%02i", G_TicsToMinutes(maretime, true),
             G_TicsToSeconds(maretime), G_TicsToCentiseconds(maretime)));
      V_DrawRightAlignedString(
          BASEVIDWIDTH / 2 + 48, 148, aflag,
          va("%d", (stplyr->finishedtime / TICRATE) * 100));
    }
    break;
  }
  ...
  case NTV_BONUSTIMEEND: // End Bonus
  {
    ...
    V_DrawString(BASEVIDWIDTH / 2 - 48, 140, V_YELLOWMAP | aflag,
                 (G_IsSpecialStage(gamemap)) ? "SPHERES:" : "CHIPS:");
    V_DrawString(BASEVIDWIDTH / 2 - 48, 148, V_YELLOWMAP | aflag, "BONUS:");
    V_DrawRightAlignedString(BASEVIDWIDTH / 2 + 48, 132, aflag,
                             va("%i:%02i.%02i",
                                G_TicsToMinutes(stplyr->lastmaretime, true),
                                G_TicsToSeconds(stplyr->lastmaretime),
                                G_TicsToCentiseconds(stplyr->lastmaretime)));
    V_DrawRightAlignedString(BASEVIDWIDTH / 2 + 48, 140, aflag,
                             va("%d", stplyr->finishedspheres));
    V_DrawRightAlignedString(BASEVIDWIDTH / 2 + 48, 148, aflag,
                             va("%d", stplyr->finishedspheres * 50));
    ST_DrawNightsOverlayNum((BASEVIDWIDTH / 2 + 48) << FRACBITS,
                            160 << FRACBITS, FRACUNIT, aflag,
                            stplyr->lastmarescore, nightsnum, SKINCOLOR_AZURE);

    // If this is a multi-mare map, display the mare number.
    if (stplyr->lastmare || P_FindLowestMare() < UINT8_MAX)
      V_DrawLevelActNum(BASEVIDWIDTH / 2 - 80, 128 + 3, aflag,
                        stplyr->lastmare + 1);

    // If new record, say so!
    if (!(netgame || multiplayer) &&
        G_GetBestNightsScore(gamemap, stplyr->lastmare + 1, clientGamedata) <=
            stplyr->lastmarescore) {
      if (stplyr->texttimer & 16)
        V_DrawCenteredString(BASEVIDWIDTH / 2, 184, aflag,
                             "\x85* \x82NEW RECORD \x85*\x80");
    }

    if (P_HasGrades(gamemap, stplyr->lastmare + 1)) {
      UINT8 grade =
          P_GetGrade(stplyr->lastmarescore, gamemap, stplyr->lastmare);
      if (modeattacking || !G_IsSpecialStage(gamemap) || grade >= GRADE_A)
        V_DrawTranslucentPatch(BASEVIDWIDTH / 2 + 60, 128, aflag,
                               ngradeletters[grade]);
    }
    break;
  }
...
static void ST_drawNiGHTSLink(void) {
  if (stplyr->linkcount !=
      0) // Don't show a faint 4294967295 link, even when debugging
  {
    static INT32 prevsel[2] = {0, 0}, prevtime[2] = {0, 0};
    const UINT8 q =
        ((splitscreen && stplyr == &players[secondarydisplayplayer]) ? 1 : 0);
    INT32 sel = ((stplyr->linkcount - 1) / 5) % NUMLINKCOLORS,
          aflag = V_PERPLAYER,
          mag = ((stplyr->linkcount - 1 >= 300)
                     ? (stplyr->linkcount - 1 >= 600) ? 2 : 1
                     : 0);
    ...
    if (stplyr->powers[pw_nights_linkfreeze] &&
        (!(stplyr->powers[pw_nights_linkfreeze] & 2) ||
         (stplyr->powers[pw_nights_linkfreeze] > flashingtics)))
      colornum = SKINCOLOR_ICY;
    else
      colornum = linkColor[mag][sel];

    aflag |= ((stplyr->linktimer < (UINT32)nightslinktics / 3)
                  ? (9 - 9 * stplyr->linktimer / (nightslinktics / 3))
                        << V_ALPHASHIFT
                  : 0);
    ...
    ST_DrawNightsOverlayNum(x - (4 * scale), y, scale, aflag,
                            (stplyr->linkcount - 1), nightsnum, colornum);
    V_DrawFixedPatch(x + (4 * scale), y, scale, aflag, nightslink,
                     colornum == 0 ? colormaps
                                   : R_GetTranslationColormap(
                                         TC_DEFAULT, colornum, GTC_CACHE));
  }

  // Show remaining link time left in debug
  if (cv_debug & DBG_NIGHTSBASIC)
    V_DrawCenteredString(BASEVIDWIDTH / 2, 180, V_SNAPTOBOTTOM,
                         va("End in %d.%02d", stplyr->linktimer / TICRATE,
...
static void ST_drawNiGHTSHUD(void) {
  ...
  const boolean oldspecialstage =
      (G_IsSpecialStage(gamemap) && !(maptol & TOL_NIGHTS));

  // Drill meter
  if (LUA_HudEnabled(hud_nightsdrill) &&
    ...
    V_DrawScaledPatch(locx, locy,
                      V_PERPLAYER | V_SNAPTOLEFT | V_SNAPTOBOTTOM | V_HUDTRANS,
                      drillbar);
    for (dfill = 0; dfill < stplyr->drillmeter / 20 && dfill < 96; ++dfill)
      V_DrawScaledPatch(locx + 2 + dfill, locy + 3,
                        V_PERPLAYER | V_SNAPTOLEFT | V_SNAPTOBOTTOM |
                            V_HUDTRANS,
                        drillfill[fillpatch]);

    // Display actual flyangle, drill amount, and bumper time
    if (!splitscreen && (cv_debug & DBG_NIGHTSBASIC)) {
      V_DrawString(locx, locy - 16, V_MONOSPACE,
                   va("ANGLE: %3d", stplyr->flyangle));

      if (stplyr->bumpertime)
        V_DrawString(
            locx, locy - 8, V_REDMAP | V_MONOSPACE,
            va("BUMPER: 0.%02d", G_TicsToCentiseconds(stplyr->bumpertime)));
      else
        V_DrawString(
            locx, locy - 8, V_MONOSPACE,
            va("DRILL: %3d%%", (stplyr->drillmeter * 100) / (96 * 20)));
    }
  }
  ...
  // Link drawing
  if (!oldspecialstage
      // Don't display when the score is showing (it popping up for a split
      // second when exiting a map is intentional)
      && !(stplyr->texttimer && stplyr->textvar == NTV_BONUSTIMEEND) &&
      LUA_HudEnabled(hud_nightslink) &&
      ((cv_debug & DBG_NIGHTSBASIC) ||
       stplyr->linkcount > 1)) // When debugging, show "0 Link".
  {
    ST_drawNiGHTSLink();
  }
  ...
  if (LUA_HudEnabled(hud_nightsspheres)) {
    ...
    if (G_IsSpecialStage(gamemap))
      ST_DrawTopLeftOverlayPatch(
          24, 16,
          ((stplyr->bonustime && (leveltime & 4) &&
            (states[S_BLUESPHEREBONUS].frame & FF_ANIMATE))
               ? nssbon
               : nsshud));
    else
      ST_DrawTopLeftOverlayPatch(
          24, 16,
          *(((stplyr->bonustime) ? nbon : nhud) + ((leveltime / 2) % 12)));
    ...
    if (!P_MobjWasRemoved(stplyr->capsule)) {
      ...
      if (stplyr->capsule->reactiontime != 0) {
        ...
        for (r = 0; r < 5; r++) {
          V_DrawScaledPatch(230 - (7 * r), 144, V_PERPLAYER | V_HUDTRANS,
                            redstat);
          V_DrawScaledPatch(188 - (7 * r), 144, V_PERPLAYER | V_HUDTRANS,
                            orngstat);
          V_DrawScaledPatch(146 - (7 * r), 144, V_PERPLAYER | V_HUDTRANS,
                            yelstat);
          V_DrawScaledPatch(104 - (7 * r), 144, V_PERPLAYER | V_HUDTRANS,
                            byelstat);
        }
        ...
        if (amount > 0) {
          ...
          for (r = amount; r > 0; r--) {
            ...
            V_DrawScaledPatch(69 + (7 * t), 144, V_PERPLAYER | V_HUDTRANS,
                              bluestat);
          }
        }
      } else {
        ...
        // Lil' white box!
        V_DrawScaledPatch(15, 8 + 34,
                          V_PERPLAYER | V_SNAPTOLEFT | V_SNAPTOTOP | V_HUDTRANS,
                          capsulebar);
        ...
        for (cfill = 0; cfill < amount && cfill < length; ++cfill)
          V_DrawScaledPatch(15 + cfill + 1, 8 + 35,
    ...
    if (oldspecialstage) {
      ...
      if (nummaprings > 0) // don't count down if there ISN'T a valid maximum
                           // number of rings, like sonic 3
      {
      ...
      V_DrawScaledPatch(272, 8,
                        V_PERPLAYER | V_SNAPTOTOP | V_SNAPTORIGHT | V_HUDTRANS,
                        nbracket);
      V_DrawScaledPatch(280, 16 + 1,
                        V_PERPLAYER | V_SNAPTOTOP | V_SNAPTORIGHT | V_HUDTRANS,
                        nring);
      V_DrawScaledPatch(280, 8 + 5,
                        V_FLIP | V_PERPLAYER | V_SNAPTOTOP | V_SNAPTORIGHT |
                            V_HUDTRANS,
                        narrow[8]);
      V_DrawTallNum(272, 8 + 11,
                    V_PERPLAYER | V_SNAPTOTOP | V_SNAPTORIGHT | V_HUDTRANS,
                    total_ringcount);
    }

    if (total_spherecount >= 100)
      V_DrawTallNum((total_spherecount >= 1000) ? 76 : 72, 8 + 11,
                    V_PERPLAYER | V_SNAPTOTOP | V_SNAPTOLEFT | V_HUDTRANS,
                    total_spherecount);
    else
      V_DrawTallNum(68, 8 + 11,
                    V_PERPLAYER | V_SNAPTOTOP | V_SNAPTOLEFT | V_HUDTRANS,
                    total_spherecount);
  }
  ...
  if (!stplyr->exiting && !oldspecialstage && LUA_HudEnabled(hud_nightsscore))
    ST_DrawNightsOverlayNum(304 << FRACBITS, 14 << FRACBITS, FRACUNIT,
                            V_PERPLAYER | V_SNAPTOTOP | V_SNAPTORIGHT,
                            stplyr->marescore, nightsnum, SKINCOLOR_AZURE);
...
#define VFLAGS V_SNAPTOTOP | V_SNAPTORIGHT | V_PERPLAYER | V_HUDTRANS
      V_DrawScaledPatch(BASEVIDWIDTH - 16, 40, VFLAGS,
                        W_CachePatchName("NGRTIMER", PU_HUDGFX));
      V_DrawPaddedTallNum(BASEVIDWIDTH - 16, 40, VFLAGS,
                          G_TicsToCentiseconds(maretime), 2);
      ...
      if (maretime < 60 * TICRATE)
        ...
        V_DrawPaddedTallNum(BASEVIDWIDTH - 40, 40, VFLAGS,
                            G_TicsToSeconds(maretime), 2);
        V_DrawScaledPatch(BASEVIDWIDTH - 64, 40, VFLAGS, sbocolon);
        V_DrawTallNum(BASEVIDWIDTH - 64, 40, VFLAGS,
                      G_TicsToMinutes(maretime, true));
      }
#undef VFLAGS
    }
  }
  ...
  if (!stplyr->exiting && stplyr->nightstime > 0 &&
    ...
    if (G_IsSpecialStage(gamemap)) {
      ...
      for (i = 0; i < MAXPLAYERS; i++)
        if (playeringame[i] && players[i].powers[pw_carry] == CR_NIGHTSMODE &&
            players[i].nightstime < lowest_time)
    ...
    if (stplyr->powers[pw_flashing] > TICRATE) // was hit
    {
      ...
      {
        UINT32 fadingFlag = (9 - 9 * flashingLeft / (TICRATE / 2))
                            << V_ALPHASHIFT;
        V_DrawTranslucentPatch(160 - (minus5sec->width / 2), 28,
                               V_PERPLAYER | fadingFlag, minus5sec);
      } else
        V_DrawScaledPatch(160 - (minus5sec->width / 2), 28, V_PERPLAYER,
                          minus5sec);
    }
    ...
    if ((oldspecialstage && leveltime & 2) &&
        (stplyr->mo->eflags & (MFE_TOUCHWATER | MFE_UNDERWATER) &&
         !(stplyr->powers[pw_shield] &
           ((stplyr->mo->eflags & MFE_TOUCHLAVA) ? SH_PROTECTFIRE
                                                 : SH_PROTECTWATER))))
      col = SKINCOLOR_ORANGE;

    ST_DrawNightsOverlayNum((160 + numbersize) << FRACBITS, 14 << FRACBITS,
                            FRACUNIT, V_PERPLAYER | V_SNAPTOTOP, realnightstime,
                            nightsnum, col);

    // Show exact time in debug
    if (cv_debug & DBG_NIGHTSBASIC)
      V_DrawString(160 + numbersize + 8, 24,
                   V_SNAPTOTOP |
                       ((realnightstime < 10) ? V_REDMAP : V_YELLOWMAP),
                   va("%02d", G_TicsToCentiseconds(stplyr->nightstime)));
  }
  ...
  if (oldspecialstage && LUA_HudEnabled(hud_nightsrecords)) {
    if (leveltime < 5 * TICRATE) {
      ...
      V_DrawCenteredString(BASEVIDWIDTH / 2, 60, aflag,
                           va(M_GetText("\x80GET\x82 %d\x80 SPHERE%s!"),
                              ssspheres, (ssspheres > 1) ? "S" : ""));
    }
  } else {
    ...
    if (cv_debug & DBG_NIGHTSBASIC) {
      ...
      if (stplyr->powers[pw_nights_superloop]) {
        ...
        V_DrawSmallScaledPatch(110, 44, 0,
                               W_CachePatchName("NPRUA0", PU_SPRITE));
        V_DrawThinString(
            106, 52, V_MONOSPACE,
            va("%2d.%02d", pwr / TICRATE, G_TicsToCentiseconds(pwr)));
      }

      if (stplyr->powers[pw_nights_helper]) {
        pwr = stplyr->powers[pw_nights_helper];
        V_DrawSmallScaledPatch(150, 44, 0,
                               W_CachePatchName("NPRUC0", PU_SPRITE));
        V_DrawThinString(
            146, 52, V_MONOSPACE,
            va("%2d.%02d", pwr / TICRATE, G_TicsToCentiseconds(pwr)));
      }
      ...
      if (stplyr->powers[pw_nights_linkfreeze]) {
        ...
        V_DrawSmallScaledPatch(190, 44, 0,
                               W_CachePatchName("NPRUE0", PU_SPRITE));
        V_DrawThinString(
            186, 52, V_MONOSPACE,
            va("%2d.%02d", pwr / TICRATE, G_TicsToCentiseconds(pwr)));
      }
...
static void ST_drawWeaponSelect(INT32 xoffs, INT32 y) {
  ...
  V_DrawScaledPatch(6 + xoffs, y - 2 - del / 2, V_PERPLAYER | V_SNAPTOBOTTOM,
                    curweapon);
}

static void ST_drawWeaponRing(powertype_t weapon, INT32 rwflag, INT32 wepflag,
                              INT32 xoffs, INT32 y, patch_t *pat) {
  ...
  if (stplyr->powers[weapon]) {
    ...
    if (weapon == pw_infinityring || (stplyr->ringweapons & rwflag))
      ; // txtflags |= V_20TRANS;
    else {
    ...
    V_DrawScaledPatch(8 + xoffs, y, V_PERPLAYER | V_SNAPTOBOTTOM | patflags,
                      pat);
    V_DrawRightAlignedThinString(24 + xoffs, y + 8,
                                 V_PERPLAYER | V_SNAPTOBOTTOM | txtflags,
                                 va("%d", stplyr->powers[weapon]));

    if (stplyr->currentweapon == wepflag)
      ST_drawWeaponSelect(xoffs, y);
  } else if (stplyr->ringweapons & rwflag)
    V_DrawScaledPatch(8 + xoffs, y,
                      V_PERPLAYER | V_SNAPTOBOTTOM | V_TRANSLUCENT, pat);
}
...
static void ST_drawMatchHUD(void) {
...

  {
    if (stplyr->powers[pw_infinityring])
      ...
      if (stplyr->rings > 0)
        V_DrawScaledPatch(8 + offset, y, V_PERPLAYER | V_SNAPTOBOTTOM,
                          normring);
      else
        V_DrawTranslucentPatch(
            8 + offset, y, V_PERPLAYER | V_SNAPTOBOTTOM | V_80TRANS, normring);

      if (!stplyr->currentweapon)
        ST_drawWeaponSelect(offset, y);
    }

    ST_drawWeaponRing(pw_automaticring, RW_AUTO, WEP_AUTO, offset + 20, y,
                      autoring);
    ST_drawWeaponRing(pw_bouncering, RW_BOUNCE, WEP_BOUNCE, offset + 40, y,
                      bouncering);
    ST_drawWeaponRing(pw_scatterring, RW_SCATTER, WEP_SCATTER, offset + 60, y,
                      scatterring);
    ST_drawWeaponRing(pw_grenadering, RW_GRENADE, WEP_GRENADE, offset + 80, y,
                      grenadering);
    ST_drawWeaponRing(pw_explosionring, RW_EXPLODE, WEP_EXPLODE, offset + 100,
                      y, explosionring);
    ST_drawWeaponRing(pw_railring, RW_RAIL, WEP_RAIL, offset + 120, y,
                      railring);
...
#define textHUDdraw(str)                                                       \
  {                                                                            \
    V_DrawThinString(                                                          \
        16, y, V_PERPLAYER | V_HUDTRANS | V_SNAPTOLEFT | V_SNAPTOTOP, str);    \
    y += 8;                                                                    \
  }
  ...
  if (stplyr->spectator &&
      (!G_CoopGametype() || stplyr->playerstate == PST_LIVE))
    textHUDdraw(M_GetText("\x86"
                          "Spectator mode:"))
      ...
      if (stplyr->exiting)
        textHUDdraw(
            M_GetText("\x82"
                      "FINISHED!")) else textHUDdraw(va("Lap:"
                                                        "\x82 %u/%d",
                                                        stplyr->laps + 1,
                                                        cv_numlaps.value))
    }
  ...
  if (!G_CoopGametype() &&
      (stplyr->exiting ||
       (G_GametypeUsesLives() && stplyr->lives <= 0 && countdown != 1))) {
    if (!splitscreen && !donef12) {
      textHUDdraw(M_GetText("\x82"
                            "VIEWPOINT:"
                            "\x80 Switch view")) donef12 = true;
    }
  } else if ((gametyperules & GTR_RESPAWNDELAY) &&
             stplyr->playerstate == PST_DEAD &&
             stplyr->lives) // Death overrides spectator text.
  {
    INT32 respawntime = cv_respawntime.value - stplyr->deadtimer / TICRATE;

    if (respawntime > 0 && !stplyr->spectator)
      textHUDdraw(va(M_GetText("Respawn in %d..."),
                     respawntime)) else textHUDdraw(M_GetText("\x82"
                                                              "JUMP:"
                                                              "\x80 Respawn"))
  } else if (stplyr->spectator &&
             (!G_CoopGametype() || stplyr->playerstate == PST_LIVE)) {
    ...
    textHUDdraw(M_GetText("\x82"
                          "JUMP:"
                          "\x80 Rise")) textHUDdraw(M_GetText("\x82"
                                                              "SPIN:"
                                                              "\x80 Lower"))

        if (G_IsSpecialStage(gamemap)) textHUDdraw(M_GetText(
            "\x82"
            "Wait for the stage to end...")) else if (G_PlatformGametype()) {
      if (G_GametypeUsesCoopLives()) {
        if (stplyr->lives <= 0 && cv_cooplives.value == 2 &&
            (netgame || multiplayer)) {
          INT32 i;
          for (i = 0; i < MAXPLAYERS; i++) {
            if (!playeringame[i])
          ...
          if (i != MAXPLAYERS)
            textHUDdraw(M_GetText(
                "You'll steal a life on "
                "respawn...")) else textHUDdraw(M_GetText("Wait to respawn..."))
        } else
  ...
  if (G_CoopGametype() &&
      (!stplyr->spectator ||
       (!(maptol & TOL_NIGHTS) && G_IsSpecialStage(gamemap))) &&
      (stplyr->exiting || (stplyr->pflags & PF_FINISHED))) {
    ...
    if (numneeded) {
      ...
      if (exiting < total) {
        ...
        total -= exiting;
        textHUDdraw(va(M_GetText("%d player%s remaining"), total,
                       ((total == 1) ? "" : "s")))
      }
    }
    ...
    if (leveltime < hidetime * TICRATE) {
      if (stplyr->pflags & PF_TAGIT) {
        ...
        textHUDdraw(M_GetText("Hide before time runs out!")) else textHUDdraw(
            M_GetText("Flee before you are hunted!"))
    } else if ((gametyperules & GTR_HIDEFROZEN) &&
...
#define SEP 20
  ...
  if (LUA_HudEnabled(hud_teamscores)) {
    if (gametyperules & GTR_TEAMFLAGS) {
      V_DrawSmallScaledPatch(BASEVIDWIDTH / 2 - SEP - (bflagico->width / 4), 4,
                             V_HUDTRANS | V_PERPLAYER | V_SNAPTOTOP, bflagico);
      V_DrawSmallScaledPatch(BASEVIDWIDTH / 2 + SEP - (rflagico->width / 4), 4,
                             V_HUDTRANS | V_PERPLAYER | V_SNAPTOTOP, rflagico);
    } else {
      V_DrawSmallScaledPatch(BASEVIDWIDTH / 2 - SEP - (bmatcico->width / 4), 4,
                             V_HUDTRANS | V_PERPLAYER | V_SNAPTOTOP, bmatcico);
      V_DrawSmallScaledPatch(BASEVIDWIDTH / 2 + SEP - (rmatcico->width / 4), 4,
                             V_HUDTRANS | V_PERPLAYER | V_SNAPTOTOP, rmatcico);
    }
  }
    ...
    goto num;
  {
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
      ...
      if (players[i].gotflag & GF_BLUEFLAG && LUA_HudEnabled(hud_teamscores))
        V_DrawScaledPatch(BASEVIDWIDTH / 2 - SEP - (nonicon->width / 2), 0,
                          V_HUDTRANS | V_PERPLAYER | V_SNAPTOTOP, nonicon);

      // Red flag isn't at base
      if (players[i].gotflag & GF_REDFLAG && LUA_HudEnabled(hud_teamscores))
        V_DrawScaledPatch(BASEVIDWIDTH / 2 + SEP - (nonicon2->width / 2), 0,
                          V_HUDTRANS | V_PERPLAYER | V_SNAPTOTOP, nonicon2);

      whichflag |= players[i].gotflag;

      if ((whichflag & (GF_REDFLAG | GF_BLUEFLAG)) ==
          (GF_REDFLAG | GF_BLUEFLAG))
        break; // both flags were found, let's stop early
    }

    // Display a countdown timer showing how much time left until the flag
    // returns to base.
    {
      if (blueflag && blueflag->fuse > 1 && LUA_HudEnabled(hud_teamscores))
        V_DrawCenteredString(BASEVIDWIDTH / 2 - SEP, 8,
                             V_YELLOWMAP | V_HUDTRANS | V_PERPLAYER |
                                 V_SNAPTOTOP,
                             va("%u", (blueflag->fuse / TICRATE)));

      if (redflag && redflag->fuse > 1 && LUA_HudEnabled(hud_teamscores))
        V_DrawCenteredString(BASEVIDWIDTH / 2 + SEP, 8,
                             V_YELLOWMAP | V_HUDTRANS | V_PERPLAYER |
                                 V_SNAPTOTOP,
                             va("%u", (redflag->fuse / TICRATE)));
    }
  }

num:
  if (LUA_HudEnabled(hud_teamscores))
    V_DrawCenteredString(BASEVIDWIDTH / 2 - SEP, 16,
                         V_HUDTRANS | V_PERPLAYER | V_SNAPTOTOP,
                         va("%u", bluescore));
...
/*static void ST_drawSpecialStageHUD(void)
{
        if (ssspheres > 0)
        {
                if (hudinfo[HUD_SS_TOTALRINGS].x)
                        ST_DrawNumFromHud(HUD_SS_TOTALRINGS, ssspheres,
V_HUDTRANS); else if (cv_timetic.value == 2)
                        V_DrawTallNum(hudinfo[HUD_RINGSNUMTICS].x,
hudinfo[HUD_SS_TOTALRINGS].y,
hudinfo[HUD_RINGSNUMTICS].f|V_PERPLAYER|V_HUDTRANS, ssspheres); else
                        V_DrawTallNum(hudinfo[HUD_RINGSNUM].x,
hudinfo[HUD_SS_TOTALRINGS].y, hudinfo[HUD_RINGSNUM].f|V_PERPLAYER|V_HUDTRANS,
ssspheres);
        }
        ...
        if (sstimer)
        {
                V_DrawString(hudinfo[HUD_TIMELEFT].x, hudinfo[HUD_TIMELEFT].y,
hudinfo[HUD_TIMELEFT].f|V_PERPLAYER|V_HUDTRANS, M_GetText("TIME LEFT"));
...
static INT32 ST_drawEmeraldHuntIcon(mobj_t *hunt, patch_t **patches,
                                    INT32 offset) {
  INT32 interval, i;
  if (stplyr->mo == NULL)
    return 0; // player just joined after spectating, can happen on custom
              // gamemodes.
  UINT32 dist =
      ((UINT32)P_AproxDistance(
  ...
  if (!F_GetPromptHideHud(hudinfo[HUD_HUNTPICS].y))
    V_DrawScaledPatch(hudinfo[HUD_HUNTPICS].x + offset, hudinfo[HUD_HUNTPICS].y,
                      hudinfo[HUD_HUNTPICS].f | V_PERPLAYER | V_HUDTRANS,
                      patches[i]);
  return interval;
}
...
static void ST_doHuntIconsAndSound(void) {
  ...
  if (!(P_AutoPause() || paused) && interval > 0 && leveltime &&
      leveltime % interval == 0 && renderisnewtic)
...
static boolean ST_doItemFinderIconsAndSound(void) {
  ...
  if (!(cv_itemfinder.value &&
        M_SecretUnlocked(SECRET_ITEMFINDER, clientGamedata))) {
    // Not unlocked, or not enabled. Use emerald hunt radar.
    return false;
  }

  for (i = 0; i < numemblems; ++i) {
    if (emblemlocations[i].type > ET_SKIN ||
        emblemlocations[i].level != gamemap)
      continue;
    ...
    for (i = 0; i < stemblems; ++i) {
      if (mo2->health == emblems[i] + 1) {
        if (P_EmblemWasCollected(emblems[i]) ||
            !P_CanPickupEmblem(stplyr, emblems[i]))
          break;
...
static void ST_overlayDrawer(void) {
  ...
  if (!(hu_showscores && (netgame || multiplayer))) {
    if ((maptol & TOL_NIGHTS || G_IsSpecialStage(gamemap)) &&
      ...
      // Draw "Listening..." indicator below RINGS
      if (ai_listening) {
        V_DrawString(hudinfo[HUD_RINGS].x, hudinfo[HUD_RINGS].y + 16,
                     hudinfo[HUD_RINGS].f | V_YELLOWMAP | V_ALLOWLOWERCASE |
                         V_PERPLAYER,
                     "LISTENING...");
      }
  ...
  // GAME OVER hud
  if (G_GametypeUsesCoopLives() && (netgame || multiplayer) &&
      (cv_cooplives.value == 0))
    ;
  else if ((G_GametypeUsesLives() ||
            ((gametyperules & (GTR_RACE | GTR_LIVES)) == GTR_RACE)) &&
           stplyr->lives <= 0 && !(hu_showscores && (netgame || multiplayer))) {
    INT32 i = MAXPLAYERS;
    INT32 deadtimer =
        stplyr->spectator ? TICRATE : (stplyr->deadtimer - (TICRATE << 1));
    ...
    if (i == MAXPLAYERS && deadtimer >= 0) {
      ...
      UINT32 flags =
          V_PERPLAYER | (stplyr->spectator ? V_HUDTRANSHALF : V_HUDTRANS);

      V_DrawScaledPatch(lvlttlx - 8, BASEVIDHEIGHT / 2, flags,
                        (countdown == 1 ? slidtime : slidgame));
      V_DrawScaledPatch(BASEVIDWIDTH + 8 - lvlttlx, BASEVIDHEIGHT / 2, flags,
                        slidover);
    }
  }
  ...
  if (!hu_showscores) // hide the following if TAB is held
  {
    ...
    if (countdown > 1) {
      ...
      if (time < 4)
        ...
        V_DrawTallNum((BASEVIDWIDTH / 2) + width,
                      ((3 * BASEVIDHEIGHT) >> 2) - 7, V_PERPLAYER, time);
        // V_DrawCenteredString(BASEVIDWIDTH/2, 176, V_PERPLAYER, va("%d",
        // countdown/TICRATE + 1));
      }
    }

    // If you are in overtime, put a big honkin' flashin' message on the screen.
    if (((gametyperules & GTR_TIMELIMIT) && (gametyperules & GTR_OVERTIME)) &&
        cv_overtime.value && (leveltime > (timelimitintics + TICRATE / 2)) &&
        cv_timelimit.value && (leveltime / TICRATE % 2 == 0))
      V_DrawCenteredString(BASEVIDWIDTH / 2, 184, V_PERPLAYER,
                           M_GetText("OVERTIME!"));
    ...
    if (!P_IsLocalPlayer(stplyr)) {
      ...
      V_DrawCenteredString((BASEVIDWIDTH / 6), BASEVIDHEIGHT - 80, 0,
                           M_GetText("Viewpoint:"));
      V_DrawCenteredString((BASEVIDWIDTH / 6), BASEVIDHEIGHT - 64,
  ...
  if ((cv_showinput.value && !players[displayplayer].spectator) ||
      (modeattacking && !(demoplayback && hu_showscores)))
    ST_drawInput();
...
void ST_Drawer(void) {
  if (cv_seenames.value && cv_allowseenames.value &&
      displayplayer == consoleplayer && seenplayer && seenplayer->mo) {
    INT32 c = 0;
    ...
    default:
      // Green = Ally, Red = Foe
      if (G_GametypeHasTeams())
        c = (players[consoleplayer].ctfteam == seenplayer->ctfteam) ? V_GREENMAP
                                                                    : V_REDMAP;
    ...
    V_DrawCenteredString(BASEVIDWIDTH / 2, BASEVIDHEIGHT / 2 + 15,
                         V_HUDTRANSHALF | c,
                         player_names[seenplayer - players]);
  }

  // Doom's status bar only updated if necessary.
  // However, ours updates every frame regardless, so the "refresh" param was
  // removed
  //(void)refresh;
  ...
  // Blindfold!
  if ((gametyperules & GTR_BLINDFOLDED) && (leveltime < hidetime * TICRATE)) {
    if (players[displayplayer].pflags & PF_TAGIT) {
      stplyr = &players[displayplayer];
      V_DrawFill(0, 0, BASEVIDWIDTH, BASEVIDHEIGHT, 31 | V_PERPLAYER);
    } else if (splitscreen &&
               players[secondarydisplayplayer].pflags & PF_TAGIT) {
      stplyr = &players[secondarydisplayplayer];
      V_DrawFill(0, 0, BASEVIDWIDTH, BASEVIDHEIGHT, 31 | V_PERPLAYER);
    }
  }
  ...
  if (lt_active) {
    ...
    if (!lt_exitticker)
      ...
      st_translucency =
          max(0, min((INT32)lt_exitticker - 4, cv_translucenthud.value));
  } else {
